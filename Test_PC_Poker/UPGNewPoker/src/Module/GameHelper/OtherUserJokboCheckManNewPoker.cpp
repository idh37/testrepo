#include "stdafx.h"
#include "OtherUserJokboCheckManNewPoker.h"
...
COtherUserJokboCheckManNewPoker::COtherUserJokboCheckManNewPoker()
{
	Clear();
	m_bUse = false;
}
...
bool COtherUserJokboCheckManNewPoker::get_Pair( int pnum )
{
	if( pnum <= 0 || 
		pnum >= MAX_POSSIBLE_PLAYER || 
		m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum <= 1 )
	{
	...
	int opencardnum = m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum;
...
bool COtherUserJokboCheckManNewPoker::get_One( int pnum )
{
	if( pnum <= 0 || 
		pnum >= MAX_POSSIBLE_PLAYER || 
		m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum <= 1 )
...
bool COtherUserJokboCheckManNewPoker::get_Two( int pnum )
{
	if( pnum <= 0 || 
		pnum >= MAX_POSSIBLE_PLAYER || 
		m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum <= 3 )
	{
		if( pnum > 0 && pnum < MAX_POSSIBLE_PLAYER ){
...
bool COtherUserJokboCheckManNewPoker::get_Three( int pnum )
{
	if( pnum <= 0 || 
		pnum >= MAX_POSSIBLE_PLAYER || 
		m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum <= 2 )
	{
		if( pnum > 0 && pnum < MAX_POSSIBLE_PLAYER ){
...
bool COtherUserJokboCheckManNewPoker::get_Straight( int pnum )
{
	if( pnum <= 0 || 
		pnum >= MAX_POSSIBLE_PLAYER || 
		m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum <= 2 )
...
bool COtherUserJokboCheckManNewPoker::get_Flush( int pnum )
{
	if( pnum <= 0 || 
		pnum >= MAX_POSSIBLE_PLAYER || 
		m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum <= 2 )
...
bool COtherUserJokboCheckManNewPoker::get_House( int pnum )
{
	if( pnum <= 0 || 
		pnum >= MAX_POSSIBLE_PLAYER || 
		m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum <= 4 )
	{
		if( pnum > 0 && pnum < MAX_POSSIBLE_PLAYER ){
...
bool COtherUserJokboCheckManNewPoker::get_Four( int pnum )
{
	if( pnum <= 0 || 
		pnum >= MAX_POSSIBLE_PLAYER || 
		m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum <= 3 )
...
bool COtherUserJokboCheckManNewPoker::get_St_Flush( int pnum )
{
	if( pnum <= 0 || 
		pnum >= MAX_POSSIBLE_PLAYER || 
		m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum <= 2 )
...
bool COtherUserJokboCheckManNewPoker::get_Rst_Flush( int pnum )
{
	if( pnum <= 0 || 
		pnum >= MAX_POSSIBLE_PLAYER || 
		m_Other_User[pnum].m_cBaseCardInfo.m_nTotalCardNum <= 2 )
...
void COtherUserJokboCheckManNewPoker::Init(NMBASE::GRAPHICGDI::xSprite *pArrowSpr)
{
	COtherUserJokboCheckMan::Init(pArrowSpr);
	m_bUse = true;
}

void COtherUserJokboCheckManNewPoker::Clear(void)
{
...
void COtherUserJokboCheckManNewPoker::Draw(CDC *pDC , NMBASE::GRAPHICGDI::CPage *pPage)
{
	...
	{
		if ( GAME()->GetPlayerNP(i)->PlayState == 1 && GAME()->GetPlayerNP(i)->JoinState == 1 && GAME()->GetPlayerNP(i)->bFold == FALSE )
		{
			POSSIBLE_CARD_COLOR hivalue_color = en_Value_Color_None;
			POSSIBLE_VALUE hi_value = en_Value_None;
			...
			if( m_Other_User[i].get_MadeCardInfo( hi_value ,hivalue_color ) )
			{
				...
				{
					if( !m_Other_User[i].IsSameCard( hi_value , GAME()->GetPlayerNP(i)->m_listCard[j].GetCardNo() ) )
					{
						continue;
					}

					ptArrowPos = GAME()->GetPlayerNP(i)->m_listCard[j].GetPos() + g_cUIData.m_ptJokboArrowGab;
...
void COtherUserJokboCheckManNewPoker::CheckJokbo(void)
{
	if( g_ObserverMan.IsMyObserver() || GAME()->GetPlayerNP(0)->JoinState == 0 ){
		Clear();
		return;
	}

	const int nMaxPlayr = GAME()->GetMaxPlayer();
	Clear();
	//자기 자신 빼고 카드 정보(오픈된 카드) 다 넣음
	for( int i = 1 ; i < nMaxPlayr ; i++ ){		
		if ( GAME()->GetPlayerNP(i)->PlayState == 1 && GAME()->GetPlayerNP(i)->JoinState == 1 && GAME()->GetPlayerNP(i)->bFold == FALSE )
		{
			int pnum = GAME()->GetServPNum_ByPN(i);
			int card_cnt = 0;
