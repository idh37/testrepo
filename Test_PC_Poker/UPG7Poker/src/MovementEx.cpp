...
void CMovementEx::StartMove(CPoint ptStart, CPoint ptEnd, DWORD dwDelay, DWORD dwMove, float fPlayProportion)
{
	ReSet();
	...
	m_dwEndTime = m_dwStartTime + dwMove;
	m_dwMoveTime = dwMove;
	m_dwGab = 0;
...
{
	if(!IsMoveStart() || IsMoveEnd()) return;
	switch(m_nType)
	{
	case MT_NORMAL:
		break;
