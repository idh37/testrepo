// GameProcess7Poker.cpp: implementation of the CGameProcess7Poker class.
//
//////////////////////////////////////////////////////////////////////
...
CGameProcess7Poker::CGameProcess7Poker(CGame *pGame)
: CGameProcess(pGame)
{
	m_pPage = NULL;
	m_bOnceInit = FALSE;
	...
	// 7포커만 m_dwDealingTimeGab을 230으로 바꾼다. 게임사업1팀 요청
	// 2012. 05. 10 최종학
	// 7포커만 m_dwDealingTimeGab을 240으로 바꾼다. 게임사업1팀 요청
	// 2012. 08. 14 류승민
	g_cUIData.m_dwDealingTimeGab = 240;
#ifdef UPGRADE_20120502_TIMING_TEST_CODE
//#ifdef _DEBUG
	g_cUIData.m_dwStartGameDealingTimeGab = GetPrivateProfileInt("7Poker", "BetDelayTime", 180, ".\\TimingTest.ini");
	g_cUIData.m_dwDealingTimeGab = GetPrivateProfileInt("7Poker", "CardStartGabTime", 220, ".\\TimingTest.ini");
//#endif //_DEBUG
#endif //UPGRADE_20120502_TIMING_TEST_CODE
}

CGameProcess7Poker::~CGameProcess7Poker()
{
	Clear();
}
...
// 칩을 그리는 함수
void CGameProcess7Poker::chip_draw_func(int PNum, INT64 Money)
{
...
// 최대 카드 개수
int  CGameProcess7Poker::GetTotalCardNum()
{
	return 7;
}
...
// 마지막 베팅 금액
INT64 CGameProcess7Poker::GetLastBetMoney(int nPlayerNum, int betCmd)
{
	INT64 Betmoney = 0;
	INT64 CallMoney = 0;
	INT64 DadangMoney = 0;
...
// ### [ Frame++ ] ###
void CGameProcess7Poker::SetWindCardEnd(BOOL bFirst)
{	
...
// 경기중 베팅 머니
INT64 CGameProcess7Poker::GetRaiseBat()
{
	INT64 nRaiseBat = 0;
...
// 실제 베팅 머니
INT64 CGameProcess7Poker::GetRealBet()
{
	INT64 nRealBet = 0;
...
// 합계 베팅 머니
INT64 CGameProcess7Poker::GetTotalBet()
{
	INT64 nRealBet = 0;
...
// 최대 머니
INT64 CGameProcess7Poker::GetMaxMoney(INT64 nTotal,int betkind )
{
	double fBatRate = 0.5;
...
// 종료시 안내 메세지 삽입 부분
void CGameProcess7Poker::Insert_EndingTextInformation(CString str, COLORREF tcolor)
{
...
// 채팅창에 정보 메시지를 출력한다.
void CGameProcess7Poker::Draw_TextInformationMsg()
{
	if ( m_GameEndMsgData.TextMsgCnt <= 0)
	{
		return;
...
// 초기화
void CGameProcess7Poker::Init(NMBASE::GRAPHICGDI::CPage *ppage)
{
	...
	//오픈애니메이션추가
int CGameProcess7Poker::GetCardOpneGuideIndex()
{
	 int a = g_Config.nbShowHidden;
...
//오픈애니메이션추가
void CGameProcess7Poker::EndCardOpenGuide()
{ 
...
void CGameProcess7Poker::SetCardOpenGuideTime()
{
	m_bDrawCardOpenGuide = TRUE;
	m_dwCardOpenGuideTime = timeGetTime();
}

void CGameProcess7Poker::ResetFlg()
{
	m_nGameCount = 0;
	m_bGameStart = FALSE;
...
// 게임 데이터를 초기화 한다.
void CGameProcess7Poker::ResetGame()
{
	ResetFlg();
...
// 모든 프로세스는 이곳에서 한다.
void CGameProcess7Poker::OnTimer()
{
	m_nGameCount++;
...
BOOL CGameProcess7Poker::OnMouseMove(int x, int y)
{
...
BOOL CGameProcess7Poker::OnLButtonDblclk(int x, int y)
{
	return FALSE;
}

BOOL CGameProcess7Poker::OnLButtonUp(int x, int y)
{
...
BOOL CGameProcess7Poker::OnLButtonDown(int x, int y)
{
...
void CGameProcess7Poker::OnGameKey(int nVKey, int nextended, int repeat)	
{	
...
void CGameProcess7Poker::OnCheckTurnLimitTimeBase()
{
...
void CGameProcess7Poker::EndWindCard()
{
...
// 방만들기 허가
bool CGameProcess7Poker::Accept_CreateRoom(void *pMsgParam) 
{
	// 스프라이트 로딩 후 초기화 ~
...
// 방에 입장 허가 받음
bool CGameProcess7Poker::Accept_EnterRoom(void *pMsgParam)
{
...
// 유저가 방에 입장 알림
void CGameProcess7Poker::User_EnterRoom(void *pMsgParam)
{
...
// 유저가 방에서 나감 알림
void CGameProcess7Poker::User_OutRoom(void *pMsgParam) 
{
...
void CGameProcess7Poker::GameStart(STARTINFO *pSC)
{
	if (pSC == NULL)
		return;
	...
	//g_pGameView->m_cBackGround.OnStartGame();
}


// 카드 분배(게임시작 및 Raise에서 호출)
void CGameProcess7Poker::Card_Distribute()
{
	if (!Is7Poker() || RSTATE_NONE == g_Poker.nState) 
	{
		return; 
	}
...
//레이스가 발생하면 서버에서 ReceiveSV_RAISE호출 pMsg는 베팅버튼정보
void CGameProcess7Poker::Raise(POKERGAME *pMsg)
{
...
// 서버에서 받은 베팅 버튼 및 금액 관련된 처리를 담당한다.
void CGameProcess7Poker::RaiseBase(POKERGAME *pMsg)
{
	if (pMsg == NULL)
	{
		return;
	...
	//규제안 : 맥스버튼 초기화
	g_pGameView->m_cGameViewBtnMananager.ResetMaxBtn();
	...
	for (int i = 0; i < MAX_PLAYER; ++i)
	{
		...
		{  
			...
			ErrorLogConsole("-------------RaiseBase[%d:%d] %I64d = %I64d", i, nPlayerNum, g_Poker.RU[i].nPMoney, g_Poker.RU[i].nRoundChip);
	...
	if (g_Poker.LastPlayer >= 0 && g_Poker.LastPlayer < MAX_PLAYER)
	{
		...
		{				
			...
			if (g_Poker.RU[LastPnum].bMaxBet && GM().g_bFirstMax[LastPnum] == false)
			{
				...
				if (true == g_PLAYER(nSNum)->IsAllIn())
				{
			...
			else
			{
				...
				case RAISE_LAST_CMD_FULL:
					{
						...
						pObj->Init(nSNum,ANF_BET_CMD,ANF_BET_FULL);//규제안 : PokerCommon/data/BettingEffect.spr에서 멀티씬 아이디 수정
#else
						pObj->Init(nSNum,ANF_BET_CMD,ANF_BET_DADANG);//규제안 : PokerCommon/data/BettingEffect.spr에서 멀티씬 아이디 수정
#endif
						cpEffectPos=pObj->GetEffectPos();
				...
				case RAISE_LAST_CMD_ALLIN:
					{					
						...
						/*
						if (g_PLAYER(nSNum)->UI.GetMoney() == 0 )
						{	
							g_pGameView->m_cPlayerDrawManager.SetPlayAllinAni(nSNum, timeGetTime());
						}
						*/
				...
				// 배팅 커맨드에 따른 사운드 재생
				// 카드세팅 값에 의한 딜링일 때는 사운드 안나옴.
				if ( CheckReceivedAllCards() )
				{
					if ( nLastCmd >= RAISE_LAST_CMD_CALL && nLastCmd <= RAISE_LAST_CMD_ALLIN && g_Poker.bDistCard == FALSE )
					{
						// 히든
						if (g_Poker.nRound == 4)
						{
...
// 게임 오버
void CGameProcess7Poker::OnGameOver(GAMEOVERRESULT *pGOR)
{	
...
//#endif
		...
		if (strlen(pGOR->Ui[i].ID) > 0) //  겜참여자.
		{
			...
			if ( g_pGameView->m_cPlayerDrawManager.IsOpen() && !pGOR->Pg.PS[i].bFold )
			{
				...
				else
				{
					...
					m_cPlayer[nPlayNum].SetCardFrontShow(0); // 카드 앞면 표시
					m_cPlayer[nPlayNum].SetCardFrontShow(1); // 카드 앞면 표시
					m_cPlayer[nPlayNum].SetCardFrontShow(6); // 카드 앞면 표시
				}
				
				m_cPlayer[nPlayNum].SetCardResult(); // 족보 표시
			...
			else if ( !g_pGameView->m_cPlayerDrawManager.IsOpen() && !pGOR->Pg.PS[i].bFold)
			{
				...
				if ( nPlayNum == 0 )
				{
					...
					if ( !g_ObserverMan.IsMyObserver() )
					{
						...
						m_cPlayer[nPlayNum].SetCardFrontShow(6); // 카드 앞면 표시
						m_cPlayer[nPlayNum].SetCardResult(); // 족보 표시
						m_cPlayer[nPlayNum].Draw_DarkCard(); // 족보에 해당하지 않는 카드 어둡게
					}
					else
					{
						m_cPlayer[nPlayNum].SetCardBackShow(0); // 카드 뒷면 표시
						m_cPlayer[nPlayNum].SetCardBackShow(1); // 카드 뒷면 표시
						m_cPlayer[nPlayNum].SetCardBackShow(6); // 카드 뒷면 표시
					}
				}
				else
				{
			...
			if( m_cPlayer[nPlayNum].bFoldOpen )
			{
				...
				m_cPlayer[nPlayNum].SetCardFrontShow(2); // 카드 앞면 표시
				m_cPlayer[nPlayNum].SetCardFrontShow(3); // 카드 앞면 표시
				m_cPlayer[nPlayNum].SetCardFrontShow(4); // 카드 앞면 표시
				m_cPlayer[nPlayNum].SetCardFrontShow(5); // 카드 앞면 표시
	...
	// 게임 결과를 전광판에 출력한다.
	g_pGameView->m_cDisplayBoard.SetText(EDBT_RESULT, (LPARAM)&m_GameOverResultData); 
...
void CGameProcess7Poker::OnResultGameOver()
{
	GM().SetWaitResponse(FALSE);
	...
	for (int i=0; i< GetMaxPlayer(); i++)
	{
		if (strlen(m_GameOverResultData.Ui[i].ID)>0) //  겜참여자.
		{
			...
			if (m_GameOverResultData.Ui[i].nRank < 0) 
				continue;
	...
	// 승패 이미지 그리기( 족보완성 이펙트)	
	if ( WinEftData.valuekind >= CCardRule::N7C_FLUSH && g_pGameView->m_cPlayerDrawManager.IsOpen() )
	{
...
void CGameProcess7Poker::ResultInfoDisplay(int nWinPlayerNum, GAMERESULTDATA& sWinEftData)
{
	CString str="";
	CString strM="";
	/*
	...
	*/
	...
	{
		CString str1;
		//str1.Format(" ----------------------------\n%s ----------------------------\n", g_StrMan.Get(_T("WIN_MESINFO")).GetBuffer());
		str1.Format(" %s", g_StrMan.Get(_T("WIN_MESINFO")).GetBuffer());
		str.Format(str1,m_GameOverResultData.Ui[g_pGameView->m_cPlayerDrawManager.GetWinnerPNum()].NickName);

		AddGameInfoViewEdit(str, GetChatColor(ECC2_RESULT));

		INT64 nTotBat = GetRealBet();

		strM = (CString)NMBASE::UTIL::g_MoneyMark(m_GameOverResultData.nWinMoney); // 하이 머니 
		...
		{
			// 딜러비 할인 이벤트 중일 경우 표시
			if (m_GameOverResultData.CommitionDiscount)
			{
...
// 		AddGameInfoViewEdit(str, GetChatColor(ECC2_RESULT));
	...
	{	
		...
		str = "";
		MM().Call(UPG_CMK_GET_JOKBO_NAME, (WPARAM)wpnum, (LPARAM)&str1);
		str.Format("◈ 족보: %s \n", str1.GetString());
		AddGameInfoViewEdit(str, GetChatColor(ECC2_RESULT_JOKBO));
...
// 삥 버튼
void CGameProcess7Poker::SendPingButton()
{
...
void CGameProcess7Poker::SendDdaButton()
{
	if (g_ObserverMan.IsMyObserver() == TRUE) 
		return;
...
// 하프
void CGameProcess7Poker::SendHalfButton()
{
...
// 다이
void CGameProcess7Poker::SendDieButton()
{
	if (g_ObserverMan.IsMyObserver() == TRUE)
		return;
	...
	//CString szTemp;
	//szTemp.Format("CGameProcess7Poker::SendDieButton() : 유저ID:%s \n", g_PLAYER(0)->UI.ID);
	//AddGameChatViewEdit(szTemp.GetBuffer(), GetChatColor(ECC2_RESULT_JOKBO));
	
	// 2014.2.7 crpark
	// 규제안 베팅예약 공통화
	if (CheckMyTurn(GetPlayer(0)->ServPNum, m_nCurPlayer) == false)
	{
...
// 체크
void CGameProcess7Poker::SendCheckButton()
{
	if (g_ObserverMan.IsMyObserver() == TRUE) 
		return;
...
// 콜
void CGameProcess7Poker::SendCallButton()
{
	if (g_ObserverMan.IsMyObserver() == TRUE) 
		return;
...
//G_TFT
void CGameProcess7Poker::Change_Money(CSV_CHANGE_MONEY *pMsg)
{	
...
void CGameProcess7Poker::SetPlayInfo(int nowpnum, int newpnum, BOOL bParticipation)
{
	if ( !Is7Poker() ) 
	{
		return;
		...
		if (strlen(tmpUi[i].ID) > 0)	
		{
			...
			if ( g_Poker.nState < RSTATE_RESULT )
			{
				...
				if (m_cPlayer[nServerToClientIndex].JoinState == EGAME_JOININ) 
				{
					...
					if (nTotCnt == 3 && g_RI.FormKind == CHOICE_GAME)
					{
						nTotCnt = 4;
					}
					...
					//TRACE("\n============================================================\n");
					//TRACE("SToC : %d, nTotCnt : %d, nOpCnt : %d\n",
					//	nServerToClientIndex, nTotCnt, nOpCnt);
					//TRACE("============================================================\n\n");
	...
	/*
	//맥스방, buy in, 입장머니 제한방
	if ( g_RI.bIsMaxRoom && bParticipation )
	{
		INT64 a = GM().GetMyInfo()->UI.GetSafeMoney(); 
		INT64 b = GM().GetMyInfo()->UI.GetMoney();
		INT64 c = GM().GetMyInfo()->UI.GetInMoney();

		CString strmsg;
		strmsg.Format("총 %s 중\n%s로 입장합니다.방 퇴장 후에는 다시 합산됩니다.\n",
			NMBASE::UTIL::g_MoneyMark( GM().GetMyInfo()->UI.GetMoney() ),
			NMBASE::UTIL::g_MoneyMark( GM().GetMyInfo()->UI.GetInMoney() ) );

		AddGameInfoViewEdit( strmsg, GetChatColor(ECC2_NOTIFY));  
...
BOOL CGameProcess7Poker::IsPlayingNow()
{
	return m_bGameStart;
}

//UPG작업 추가
void CGameProcess7Poker::OnEnterRoom(BOOL bCreation)
{
...
// 방에서 나간다.
void CGameProcess7Poker::OnExitRoom()
{
...
BOOL CGameProcess7Poker::PreTranslateMessage(MSG* pMsg) 
{
	///////////////////////////////////////////////////////////////
...
//UPG작업 함수추가
BOOL CGameProcess7Poker::IsSacnCode(int nScanCode, int &r_scankey, int &r_extended, int &r_previous)
{
	// 스캔코드를분석함
	// 스캔키코드
...
void CGameProcess7Poker::OnInit()
{
	set_GameSpeed();
}

void CGameProcess7Poker::set_GameSpeed()
{
	g_GameSpeed.Clear();
...
//#ifdef _DEBUG
	...
	int nBetCmdActiveDelay = GetPrivateProfileInt("7Poker", "BetBtnActiveFrameNum", 4, ".\\TimingTest.ini");
	g_GameSpeed.set_BetCmdActiveDelay(i, j, nBetCmdActiveDelay);

	nBetCmdActiveDelay = GetPrivateProfileInt("7Poker", "BetBtnActiveInAutoBetFrameNum", 4, ".\\TimingTest.ini");
...
BOOL CGameProcess7Poker::OnCommand(WPARAM wParam, LPARAM  lParam)
{
	...
	case IDM_DIE:
		{
			//AddGameChatViewEdit("CGameProcess7Poker::OnCommand()", GetChatColor(ECC2_RESULT_JOKBO));
			SendDieButton();
		}
		break;
	case IDM_CHECK:
...
// ### [사운드 추가 작업] ###
void CGameProcess7Poker::SoundFxKind(int betkind, int sex, bool bMax)
{
	//규제안 : 맥스 커맨드 추가 : 맥스이펙트
	if ( bMax )
	{
...
void CGameProcess7Poker::SoundFxKindBig(int betkind, int sex, bool bMax )
{
	//규제안 : 맥스 커맨드 추가 : 맥스이펙트
	if ( bMax )
	{
...
BOOL CGameProcess7Poker::Is7Poker()
{
...
void CGameProcess7Poker::OnCheckTurnLimitTime()
{
	if ( !Is7Poker() ) 
		return;
...
void CGameProcess7Poker::OnCardEvent(CARDEVENT *pEvent)
{
	if (pEvent == NULL)
	{
		return;
...
CPlayer* CGameProcess7Poker::GetPlayer(int i)
{
	ASSERT(i<GetMaxNewPlayer() && i>=0);
	return &m_cPlayer[i];
}

CPlayer7Poker* CGameProcess7Poker::GetPlayerNP(int i)
{
	return (CPlayer7Poker *)GetPlayer(i);
}


void CGameProcess7Poker::CheckMinimumUser(void)
{
	int nPlayer = 0;
...
void  CGameProcess7Poker::SetJackPotMoney(INT64 i64Money)
{
...
void  CGameProcess7Poker::SetOldJackPotMoney(INT64 i64Money)
{
	m_i64OldJackPotMoney = i64Money;
	g_cVipJackpot.SetShowOldJackpotMoney(m_i64JackPotMoney);
}

void CGameProcess7Poker::JackPotMoneyUpdate(jackpot::JP_MONEYINFO* pInfo)
{
	// 게임 타입
...
void CGameProcess7Poker::ClearJackpotMoney(int nGameCode, INT64 i64LimitMoney)
{
	// 게임 타입
...
void CGameProcess7Poker::TestCode(MSG* pMsg)
{
