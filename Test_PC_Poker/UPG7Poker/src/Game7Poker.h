...
class CGameProcess7Poker;
class CGameView7Poker;

// 게임 네트웍 Receive 패킷을 처리하는 클래스

class UPG_GAME_EXPORT CGame7Poker : public CGame
{
public:
	CGame7Poker(void);
	virtual ~CGame7Poker(void);
...
protected:
	...
	CGameProcess7Poker* GetGameProcess();	// 게임 프로세스 포인터
	CGameView7Poker*	GetGameView();		// 게임 뷰 포인터
...
private:
	...
	void ReceiveSV_LOSSMONEYINFO(int nTotSize, char *lpData);

	void ReceiveSV_Reward_Allin_Angel(int TotSize, char *lpData);
