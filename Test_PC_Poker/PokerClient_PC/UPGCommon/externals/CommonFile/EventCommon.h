...
// #endif // (__PROMOTION_CHANCEAVATA__)
	...
	////////////////////////////2018 추석 이벤트. 연속 출석 & 판수 보상 2018.09.20/////////////////////////////////////
	OPCODE_CL_2018THANKSGIVING_ATTENDANCE_INFO,		// 출석 이벤트. 출석부 요청.		// 응답: PROMOTION_2018THANKSGIVING_ATTEDANCE_TOTAL_INFO
	OPCODE_SV_2018THANKSGIVING_ATTENDANCE_INFO,		// 출석 이벤트. 로그인시 노티 혹은 요청(OPCODE_CL_2018THANKSGIVING_ATTENDANCE_INFO)시 응답.	// 내용: PROMOTION_2018THANKSGIVING_ATTEDANCE_TOTAL_INFO
	OPCODE_CL_2018THANKSGIVING_GAMECOUNT,			// 판수 이벤트. 현재 판수 요청			// 응답: PROMOTION_2018THANKSGIVING_GAMECOUNT
	OPCODE_SV_2018THANKSGIVING_GAMECOUNT,			// 판수 이벤트. 매판 게임 종료시 노티.	// 내용: PROMOTION_2018THANKSGIVING_GAMECOUNT
	OPCODE_SV_2018THANKSGIVING_GAMECOUNT_INFO,		// 판수 이벤트가 적용되는 채널의 정보 노티. (로그인시 1번)	내용: PROMOTION_2018THANKSGIVING_GAMECOUNT_INFO
	//////////////////////////////////////////////////////////////////////////
...
typedef struct _VISITE_PROMOTION_ALARM
{
	...
	int nRewardIndex;	//이미지처리용 당첨금 인덱스 번호이다.///////////////////////////2017.04.27 럭키휠 프로모션 작업///////////////////////////////

	INT64 nRunLowerLimit; //해당 프로모션이 작동하기 위한 채널입장 하한선
	INT64 nRunUpperLimit; //해당 프로모션이 작동하기 위한 채널입장 상한선
	//위 두개값 사이에 있어야(양 끝단 포함) 이벤트가 작동함
...
{
	int TodayGameCount; //오늘 게임한 총횟수
	int CurrentLevel; //현재 레벨.. -1이면 오늘은 종료이다.
	bool GiftOpen[4]; //각 레벨별 선물 오픈 여부이다.
	int ThisLevel_GoalCount; //목표 카운트(해당 게임 판수가 되면 레벨 성공)
	int ThisLevel_CurrentCount; //이전 레벨이후 진행된 게임판수
...
{
	int nAttendanceMonth;		// 출석 월 (9월 9)
	int nAttendanceDay;			// 출석 일 (20일 20)
	INT64 llReward;				// 출석 보상 (100억)
	INT64 llAdditionalReward;	// 추가 보상 (7일 개근시...) (500억)
	bool bIsAttendance;			// 출석 여부
	//	int nAdditionalRewardCount;	// 추가 보상 횟수(7일 개근 -> 7, 3일 개근 -> 3)
};

// 출석 보상 전체 정보
struct PROMOTION_2018THANKSGIVING_ATTEDANCE_TOTAL_INFO
{
	std::vector<PROMOTION_2018THANKSGIVING_ATTEDANCE_DAY_INFO> vecAttendanceTotalInfo;		// 현재일까지만 담아서...
	INT64 llAdditionalFinalReward;		// 최종 개근시 추가 보상 (28일 개근시...)	3000억
	//int nAdditionalRewardFinalCount;	// 최종 개근시 추가 보상 횟수(28일 개근 -> 28, 14일 개근 -> 14)
...
{
	INT64 llRunLowerLimit; //해당 프로모션이 작동하기 위한 채널입장 하한선		1
	INT64 llRunUpperLimit; //해당 프로모션이 작동하기 위한 채널입장 상한선		1000억
	//위 두개값 사이에 있어야(양 끝단 포함) 이벤트가 작동함
};
...
{
	int nTodayGameCount;		// 오늘 총 게임 횟수		(1~150)
	INT64 llReward;				// 보상(5판 마다, 없을시 0) (10, 100, 1,000, 10,000)
	INT64 llAdditionalReward;	// 추가 보상(30판 마다, 없을시 0) (500, 5,000, 10,000, 1,000,000)
};
...
{
public:
	INT64* m_llAdditionalFinalReward;			// 최종 개근시 추가 보상 (28일 개근시...)	3000억
	size_t* m_nInfoSize;
