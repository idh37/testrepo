...
CGameViewSutda::CGameViewSutda(CGame *pGame)
...
CGameViewSutda::~CGameViewSutda()
{
}


BEGIN_MESSAGE_MAP(CGameViewSutda, CGameViewBase)
	//{{AFX_MSG_MAP(CGameViewSutda)
	ON_WM_PAINT()
	ON_WM_TIMER()
	ON_WM_CREATE()
	ON_WM_DESTROY()
...
/////////////////////////////////////////////////////////////////////////////
// CGameViewSutda message handlers

BOOL CGameViewSutda::PreCreateWindow(CREATESTRUCT& cs) 
{
...
int CGameViewSutda::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CGameViewSutda::OnInit(void)
{
	if(m_bOnceInit)
	{
		return;
	...
	// 족보 출력
	//m_cGameHelper.Init(&g_cUIData, GetGame(), g_cUIData.m_nMaxJokbo, new COtherUserJokboCheckMan7Poker, new CJokboProbability7Poker);
	...
	g_SelectCard().Init(&g_cUIData, GetGame(), &g_sprBigCard, &g_sprBigCard);
	...
	{	
		m_cPlayerDrawManager.AddPlayer(g_GAMEPROCESS()->GetPlayer(nNum), new CPlayerDraw_SD, nNum);
	}
	...
	//섯다 축음기
	m_Radiophonograph.Init( this, &Page );

	m_cAskRematchPopup.Init(this, &Page);

	//섯다 칩
	m_cChipBoard.Init(&Page, m_cBackGround.GetBGSpr());
	
}


LRESULT CGameViewSutda::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch(message)
	{
	case WM_MOUSEWHEEL:
	...
	case WM_MOUSEMOVE :
		{
			...
			if(m_Radiophonograph.OnMouseMove(mxp, myp))
			{
				return TRUE; //섯다 축음기
			}
			...
			if(m_cAskRematchPopup.OnMouseMove(mxp, myp))
			{
				return TRUE;
			}
...
// 			}			
	...
	case WM_LBUTTONDOWN :
		{	
			...
			if(m_Radiophonograph.OnLButtonDown(mxp, myp)) return TRUE;	//섯다 축음기
			...
			if (m_cAskRematchPopup.OnLButtonDown(mxp, myp))
			{
				return TRUE;
			}
	...
	case WM_LBUTTONUP :
		{
			...
			if (m_cAskRematchPopup.OnLButtonUp(mxp, myp))
			{
				return TRUE;
			}
			...
			if(m_Radiophonograph.OnLButtonUp(mxp, myp)) return TRUE;	//섯다 축음기
	...
	case WM_LBUTTONDBLCLK :
		{
			...
			m_Radiophonograph.OnLButtonDbClick(mxp, myp);	//섯다 축음기
...
//	NMBASE::SOUND::g_Sound.RegularCall();
	...
	m_cChipBoard.Draw_Bottom();		//칩그리기

	m_Radiophonograph.Draw_Bottom( &MemDC );	//섯다 축음기

	


	//섯다 애니메이션 - 최저레이어 
	m_AniQue.Low_Draw( &Page, &MemDC );
	...
	m_Radiophonograph.Draw_Top( &MemDC );	//섯다 축음기
	...
	m_cChipBoard.Draw_Top(); //칩던져지는 효과
	...
	ScreenToClient(&mpos);
	CString str;
	str.Format("%5.1f FPS, Mouse (%d, %d) %s", FPS, mpos.x, mpos.y, g_RI.FormKind==SD_2CARD?"2장섯다":"3장섯다");

	MemDC.SelectObject(&NMBASE::UTIL::GetMyFont(NMBASE::UTIL::FONT_12D_NORMAL));
	MemDC.SetBkMode(TRANSPARENT);
	MemDC.SetTextColor(RGB(255,255,255));
	MemDC.TextOut(3,33, str);	 
...
void CGameViewSutda::DrawJackPot()
{		
...
void CGameViewSutda::Reset()
{
	m_bExitSubscript = FALSE;	
}

void CGameViewSutda::OnTimer(UINT nIDEvent) 
{
...
BOOL CGameViewSutda::OnCommand(WPARAM wParam, LPARAM lParam) 
{	
...
void CGameViewSutda::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
...
void CGameViewSutda::DrawPlayCard(CDC *pDC)
{	
	if(GetCurrentWhere() != IDX_GAMEWHERE_GAME)
	{
		return;
...
void CGameViewSutda::DrawPlayBottom(CDC *pDC)
{
...
void CGameViewSutda::DrawPlayMiddle(CDC *pDC)
{
...
void CGameViewSutda::DrawPlayTop(CDC *pDC)
{
...
void CGameViewSutda::DrawButton(CDC *pDC)
{
...
//2006.1.9 스피드핵
DWORD CGameViewSutda::GetCurrentPlayTick()
{
	return timeGetTime()-g_nStartTick;
}


BOOL CGameViewSutda::NoticAreaMouseOver(int x, int y )
{	
	return FALSE;
}


void CGameViewSutda::SetupByConfig()
{
...
void CGameViewSutda::ShowEffect_AllowChancePurchase(int nItemCode)
{
...
// 보유한도 달성 이펙트
void CGameViewSutda::ShowEffect_OwnLimitEffect(int nPnum)
{
...
//UPG 작업 추가
void CGameViewSutda::ShowEffect_OwnLimitIcon(BOOL bON, INT64 llMoney)
{
}


void CGameViewSutda::OnExitRoom(void)
{
}

const int CGameViewSutda::GetWidth()
{
	return 1000;
}

const int CGameViewSutda::GetHeight()
{
	return 740;
}

BOOL CGameViewSutda::OnEraseBkgnd(CDC* pDC)
{	
	OnPaint();
...
BOOL CGameViewSutda::PreTranslateMessage(MSG* pMsg)
{
...
void CGameViewSutda::GameViewSetFocus()
{
	if(this)
	{
		this->SetFocus(); 
