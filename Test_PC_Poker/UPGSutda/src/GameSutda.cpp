...
//	TTrace::Show(true) ;
	TTrace::Debug()->Send ("7Poker시작-Create", "CGameSutda");   // single byte string	
#endif
...
LRESULT	CGameSutda::SendQuickJoiner(void)
{
	...
	//	//섯다의 경우에는  내맘대로 바로입장 설정에 베팅룰에 관한 설정이 없고 시드머니 규모가 있으므로
	//	//다른게임과 달리 GetQJRoomOption 호출시 첫번째 인자로 베팅룰사용여부 대신 시드머니규모의 사용여부를 넘겨줘야 제대로된 결과를 받을 수 있다.
...
{	
	return RGB(100,89,0);//각 게임 고유의 색을 반환
}
...
#define TRACE_PACKET_CASE(sig) case sig:
	...
	TRACE_PACKET_CASE(SV_SD_49REMATCH)						// 서버에서 49 재경기 공지 (패킷 수신한 클라이언트는 참가 가능 대상자)
		RevevieSV_SD_49REMATCH(TotSize, lpData);
		break;		

	TRACE_PACKET_CASE(SV_SD_NOTIFY_REMATCH)				// 재경기 공지 (49 or 동일패)
		RevevieSV_SD_NOTIFY_REMATCH(TotSize, lpData);
		break;				

	TRACE_PACKET_CASE(SV_SD_NOTIFY_ASK)						// 재경기 참가 공지
...
void CGameSutda::ReceiveSV_DIEUSER(int nTotSize, char *lpData)
{
	...
	//char szTemp1[256] = {0, };
	//sprintf(szTemp1, "CGameSutda::ReceiveSV_DIEUSER => 플레이어:%d\n", pnum);
...
void CGameSutda::ReceiveSV_HIDDENCARD(int nTotSize, char *lpData)
{
	...
	if(g_PLAYER(pnum)->PlayState == EPLAYER_GAMEPLAYING && g_PLAYER(pnum)->JoinState == EGAME_JOININ)
	{
		g_PLAYER(pnum)->m_cMyCard[nHiddenCardIndex].SetCardNum(*msg.nCard) ; // 값 변경
	...
	g_PLAYER(0)->BackupOriginalCard();	//카드 3장 백업	
}
...
void CGameSutda::RecevieSV_SD_NOTIFY_JJOGI(int nTotSize, char *lpData)
{
	...
	if(pnum == 0 && !g_ObserverMan.IsMyObserver())	//내 쪼으기는 제외
		return;
...
{
	// 서버에서 재경기(49 or 동일패) 공지 (패킷 수신한 클라이언트는 참가 가능 대상자)
	if(GetCurrentWhere() != IDX_GAMEWHERE_GAME)
		return;
