...
enum FORMKIND
{
	...
	FORMKIND_BLIND=5, //블라인드 배팅모드이다. 로우바둑이에서 사용..
	//////////////////////////
};

////////////////////////////골드시스템,골드채널 2017.11.07/////////////////////////////////////
enum ROOM_CHIPKIND //게임을 통해 따고 잃는게 가능한 재화 타입이다.
{
	ROOM_CHIPKIND_NORMAL = 0, //그냥 어지껏 통용했던 게임머니이다.
	ROOM_CHIPKIND_GOLD, //새롭게 추가된 골드이다.
	ROOM_CHIPKIND_GOLD_BANK, //골드뱅크용 코드 추가
};
////////////////////////////////////////////////////////////////

// 골드 전적 분리. 게임 가능한 칩 타입. 2018-08-14
...
{
	//클럽 관련한 유저의 선택코드
	CLUB_ACTION_ALLOW = 0, //받아들임.. 보스의 초대, 유저의 클럽 가입등을 받아들일때..
	CLUB_ACTION_DENY, //거부함.. 보스의 초대, 유저의 클럽 가입등을 거부할때
};

enum CLUB_INVITED_TYPE
{
	//클럽 초대관련한 타입이다.
	//초대를 받은것이냐, 자발적으로 가입 요청을 한것이냐..
	CLUB_INVITED_BY_BOSS = 0, //보스의 초대
	CLUB_INVITED_BY_ME = 1, //내가 자발적으로 가입요청
};

enum CLUB_MEMBER_GRADE
{
	CLUB_MEMBER_GRADE_BOSS = 0, //클럽의 보스이다.
	CLUB_MEMBER_GRADE_NORMAL, //클럽의 일반회원이다.

	CLUB_MEMBER_GRADE_NONE = 100, //클럽에 가입하지 않은 상태이다.
};

enum CLUB_FIRE_REASON
{
	//클럽에서 강제탈퇴된 이유이다.
	CLUB_FIRE_REASON_BOSS_KICK = 0, //보스가 강제로 탈퇴 시켰다.(클럽은 존재)
	CLUB_FIRE_REASON_CLOSE, //클럽이 해산되면서 자연스레 탈퇴
};

enum COMMON_ERR_CODE
{
	//꼭 클럽에서만 사용하는건 아니고, 특정 Request에 대해 Response 할수 없는 상황에서 보내게 된다.
	COMMON_ERR_CODE_PERMISSION = 1000, //사용 권한이 없음.. 가령 클럽상세 정보를 클럽원이 아닌 사람이 요청한 경우등
	COMMON_ERR_CODE_SYSTEM, //DB서버등 관련 시스템에서 적절한 응답을 얻지 못한 경우
};

enum CLUB_MYINFO_TYPE
{
	//CLUB_MYINFO 패킷이 수시로 전달되는데, 이게 왜 가게 되는지 타입으로 나눈다.
	CLUB_MYINFO_NORMAL = 0, //로그인, 채널이동등 일반적인 상황
	CLUB_MYINFO_MY_REQUEST, //보스의 초대 수락, 자발적 탈퇴등 나의 선택에 의해 정보가 변경되어 오는 상황
	CLUB_MYINFO_OTHER_REQUEST, //내 클럽 가입에 대한 보스의 수락, 보스가 나를 강퇴, 보스의 클럽 해산등 다른 유저의 선택에 의해 정보가 변경되어 오는 상황
};
/////////////////////////////////////////////////////
...
{
	//게임판수를 이용하여 진행되는 프로모션의 구분 ID
	PROMOTION_COMMON_GAMECOUNT_PMCODE_2018_LUCKYBAG = 1, //2018년 복주머니 이벤트
	PROMOTION_COMMON_GAMECOUNT_PMCODE_2018_GOLDEN_LUCKYBAG, //2018년 황금 복주머니 이벤트
	PROMOTION_COMMON_GAMECOUNT_PMCODE_2018_NEWYEAR_CHICKEN, //2018년 새해 치킨 이벤트
};
///////////////////////////////////////////////////////////

enum PROMOTION_ODDEVEN_START_ERRCODE
{
	//홀짝프로모션 시작요청에 대한 에러코드
	//0.정상시작, 1.오늘이미진행함, 2.기타오류, 10.캡차인증코드 불일치
...
enum REASON_OUTROOM			// OnUserOutRoom 호출시 방 퇴장 이유 
{
	...
	REASON_OUTROOM_BD_MORNINGBET,			// 로우바둑이 아침벳 룰 위반
};
enum GAMESTATE
{
	GAMESTATE_NONE=0,			//
...
enum ERR_ENTERROOM
{
	...
	ERR_ENTERROOM_RD_MORNING_BET_BANNED, // 로우바둑이 아침배팅룰위반자 신청
};
...
{	
	...
	ALLGAMERECORD operator +(const ALLGAMERECORD &gr)
	{
		//이 함수는 일반 경기장과 골드 경기장의 전적을 모두 더한 전적을 반환하기 위해 사용.
		//(주의) 최대획득머니는 둘중에 최대값만 반환. GOLD는 일반 머니일때 값으로 변환해야 할지 고민해야 함.
		ALLGAMERECORD ret = *this;
	...
	UINT GetTotalNum() const /// 전체 토탈 횟수 
	{
		UINT ret=0;
		for(int r=0; r<MAX_GAMERECORD; r++) 
		{			
	...
	UINT GetTotalNum(IDX_GAME idxGame) const // 승+패+폴드횟수(게임별)
	{
		UINT ret=0;
		for(int r=0; r<MAX_GAMERECORD; r++) 
		{	
	...
	UINT GetTotalNum(IDX_GAMERECORD idxGR, IDX_GAME idxGame) const // 승+패+폴드 횟수 (게임별)
	{
...
{
	int GameCode;
	char IP[IP_LEN];		//--> 클라이언트 로컬에서 뽑은 IP
	char MAC[MAC_LEN];		//--> MAC Address
	char GameVersion[10];	//--> 클라이언트 빌드버전
	char CountryCD[4];		//--> KR, US....
	char TimeZone[6];		//--> 00:09(KR)
	char OS[30];			//--> OS 정보
	char OSCurrentBuild[8];	//--> OS CurrentBuild 버전
	char Platform[6];		//--> 32, 64 bit 정보
} FOBIS_Common;
...
typedef struct
{
	ALLGAMERECORD AllGameRecord[PlayableChipType::MAX];		// 골드 전적 분리. 게임 가능한 칩 타입 별로 전적 따로 관리. 2018-08-14
	INT64 pBackup;
...
namespace CafeSystem
{
	...
	namespace JoinResult
	{
		...
		{
			SUCCESS = 0,			// 가입 신청 완료
			ALREADY_MEMBER,			// 이미 멤버임. (가입된 상태)
			EXCESS_DAY_LIMIT_COUNT,		// 1일 초대 건수 제한 초과.			"1일 초대 10건 제한을 초과하였습니다. 익일 00시 부터 초대가 가능합니다"
			ALREADY_ASKED,			// 이미 가입 요청을 한 상태.
			FULL_MEMBER_COUNT,			// 이미 멤버가 다 찼음.
			YOURE_NOT_CAFE_BOSS,		// 카페장이 아님.
		};
	}
	...
	namespace InviteResult
	{
		...
		{
			SUCCESS = 0,				// 초대 완료
			NOT_EXIST_INVITE_MEMBER,	// 초대할 대상이 없음.
			EXCESS_DAY_LIMIT_COUNT,		// 1일 초대 건수 제한 초과.			"1일 초대 10건 제한을 초과하였습니다. 익일 00시 부터 초대가 가능합니다"
			YOURE_NOT_CAFE_BOSS,		// 카페장이 아님.
			ALREADY_MEMBER,				// 이미 멤버임. (가입된 상태)
			ALREADY_ASKED,			// 이미 가입 요청을 한 상태.
			FULL_MEMBER_COUNT,			// 이미 멤버가 다 찼음.				"카페원이 다 있는 상태입니다. 초대를 하기 위해서는 카페원이 탈퇴하거나 VIP등급이 상향되어야 합니다"
			NOW_UPDATING_VIP_GRADE,		// 월초 VIP 등급 갱신중일 경우.		"VIP 등급 갱신 중에는 카페원을 초대할 수 없습니다.  XX일 이후 다시 시도해주시기 바랍니다"
		};
	}
	...
	namespace Notice
	{
		...
		{
			NONE,
			VIPGRADE_UP_CAFE_BOSS_MORETHAN_4_GRADE,					// "VIP 등급이 상승하여 카페의 정원이 증가 하였습니다."
																	// "VIP등급이 높을수록 더욱 많은 카페원을 모집할 수 있습니다."
			VIPGRADE_UP_UNITEDCAFE_BOSS_MORETHAN_6_GRADE,			// "VIP 등급이 상승하여 연합원 보유수가 증가 하였습니다."
																	// "VIP등급이 높을수록 더욱 많은 연합원을 모집할 수 있습니다."
			VIPGRADE_DOWN_CAFE_BOSS,								// "VIP 등급이 하락하여 카페의 정원이 감소 하였습니다"
																	// "정원을 초과한 카페원은 접속일자가 오래된 유저부터 탈퇴처리 됩니다."
																	// "VIP 등급이 높을수록 더욱 많은 카페원을 모집할 수 있습니다"
			VIPGRADE_DOWN_UNITEDCAFE_BOSS,							// "VIP 등급이 하락하여 연합의 정원이 감소 하였습니다"
																	// "정원을 초과한 연합원은 접속일자가 오래된 유저부터 탈퇴처리 됩니다."
																	// "VIP 등급이 높을수록 더욱 많은 카페장을 모집할 수 있습니다"
			FIRED_CAFE_MEMBER,										// "카페장 VIP 등급이 하락하여 자동탈퇴 되었습니다."
																	// "정원을 초과한 카페원은 접속일자가 오래된 유저부터 탈퇴처리 됩니다"
			FIRED_UNITEDCAFE_MEMBER,								// "연합장 VIP 등급이 하락하여 자동탈퇴 되었습니다."
																	// "정원을 초과한 연합원은 접속일자가 오래된 유저부터 탈퇴처리 됩니다"
			MAX,
		};
	}

	// 0: 비회원 1: 준회원 2: 정회원 3: 카페장, 4: 연합원(카페장 포함) 5: 연합장 
	...
	struct CafeInfo
	{
		int cafeID_;				// 카페 ID. 0일 경우 invalid
		char cafeName_[NICK_LEN];	// 카페 이름. (생성 당시의 boss nickname)
	...
	struct MemberInfo
	{
		char memberID_[ID_LEN];		// 멤버 ID
		MemberGrade::Type memberGrade_;			// 0: 비회원 1: 준회원 2: 정회원 3: 카페장, 4: 연합원(카페장 포함) 5: 연합장 
		int globalRoomNum_;			// 게임중인 경우 방 번호. 게임중이 아니면 0. 연합방일 경우에만 담김.
...
typedef struct
{
	...
	////////////////////////////클럽시스템 2017.12.21/////////////////////////////////////
	int ClubCode; //가입한 클럽고유번호
	int ClubMemberGrade; //클럽내등급, 보스냐, 일반이냐...
	int ClubBossLevel; //보스의 VIP 등급
	char ClubName[CHAR_BUFFER_CLUB_NAME]; //가입한 클럽이름
	int ClubMyVIPLevel; //나의 VIP 등급이다.
	////////////////////////////////////////////////

	//////////////////////////////////////////////////////////////////////////
	// 카페 시스템. 2018.12.20
	CafeSystem::CafeInfo cafeInfo_;						// 카페 정보
	CafeSystem::CafeInfo unitedCafeInfo_;				// 연합 정보
	CafeSystem::MemberGrade::Type cafeMemberGrade_;		// 0: 비회원 1: 준회원 2: 정회원 3: 카페장, 4: 연합원(카페장 포함) 5: 연합장 
	//////////////////////////////////////////////////////////////////////////
    ...
    // 2018년 7월 2번째 업데이트 관련 - 본인 카드 오픈 기능 추가 
...
//#endif
	...
	bool bCardOpen; // 2018년 7월 2번째 업데이트 관련 - 본인 카드 오픈 기능 추가
	char szRegDate[20]; // 가입날짜 // 2019년 7월 30일 추가
} USERINFO;
...
typedef struct
{
	//게임 및 채널이동시 마스터서버를 통해 넘길 기타 데이터를 여기에 넣는다.
	//이 구조체는 마스터서버와 동기화 되어야 함
	INT64 Gold_RealChip;
	int Gold_RemainChip;
	INT64 Gold_OverChip;
	...
	INT64 GoldBank_RealChip; //골드통장 보유금액
	int GoldBank_RemainChip; //골드통장 남은칩
	INT64 GoldBank_OverChip; //골드통장 오버된금액
	///////////////////////////////////////////

	////////////////////////////2018년06월 오픈 2018.06.01/////////////////////////////////////
	int Gold_Insurance_Level; //골드 보험 등급
	INT64 Gold_Insurance_Money; //골드 보험 적립금
	///////////////////////////////////////////
...
//////////// NEWPOKER Add ToDo: 안쓰는 변수 제거하기
	...
	// 판수이벤트 어뷰징 체크. 현재 어뷰징 카운트를 서버 이동시에도 가져간다.
	int nCheckedAbusingCount;
	
	char cMorningBetBanishCount;			// 아침배팅 룰위반 카운트( 0 ~ 7 )
	SYSTEMTIME tmMorningBetBanishEndDate;	// 아침배팅 룰위반 벤 종료시간

	BOOL bGameAbuser; //게임 어뷰져 유저 체크 ///////////////////////////2019.04.29 럭키휠 프로모션 작업/////////////////////////////// 하면서 추가
...
typedef struct  // 대기실 사용자 정보 갱신용
{
	...
	////////////////////////////클럽시스템 2017.12.21/////////////////////////////////////
	int ClubMemberGrade; //클럽내등급, 보스냐, 일반이냐... 추가
	int ClubCode;
	char ClubName[CHAR_BUFFER_CLUB_NAME]; //가입한 클럽이름
...
typedef struct
{
	...
	INT64 MaxBetPerFriendRoomRound;	// 시행령에서 정한 한판 최대 베팅 금액 (선택입장시 적용되는 베팅 금액) - 2018년 5월 10일 작업
} RESTRAINTINFO;	
...
typedef struct
{
	...
	////////////////////////////골드시스템,골드채널 2017.11.07/////////////////////////////////////
	INT64 MaxServerMoney_Gold;					//내가 가질수 있는 최대머니(골드)
	INT64 DisplayMaxMoney_Gold[MAX_CODE_NEW_FAMILY];	// 디스플레이용 서비스별 보유 최대 한도(골드)
...
{	
	...
	int ChipKind;
	/////////////////////////////////////////
	bool bIsFriendRoom; // 친구룸 인가? // 2018년 5월 2일 랜덤매칭 시행령 관련 로직 추가

	////////////////////////////VIP그룹 분리 2018.07.16/////////////////////////////////////
	int VIPGroup; //VIP채널의 상세 그룹번호.. Default 0이고 그룹이 필요한 경우 해당 값을 사용

	//////////////////////////////////////////////////////////////////////////
	// 카페 시스템. 2018.12.20
	CafeSystem::CafeKind::Type cafeKind_;		// 카페방일 경우 셋팅.
	int cafeID_;		// 클라에서 셋팅. 연합방일 경우에만 0이 아닌값.
	//////////////////////////////////////////////////////////////////////////
	
	bool bMorningBet; // 로우바둑이 아침벳룰 ( 아침벳 중에는 하프,콜만 진행되고 다이 시 페널티 적용 )
...
typedef struct
{
	...
	BOOL bChangedEnd;		//TRUE 클라이언트 바꾸기 연출 종료 패킷을 받아야 함, FALSE 바꾸기 연출 종료 패킷 받음 
	int	 nChangedEndCnt;	//바꾸기 연출 종료 대기시간
...
typedef struct 
{
	...
	ALLGAMERECORD AllGameRecord[PlayableChipType::MAX];		//게임 각종 기록		// 2008.12 포커류 통합
	INT64	PMoney;
	int		nRank;			//등수(바둑이)
...
struct GAMEOVER_HOOLA
{
	...
	////////////////////////////훌라 기권방 2018.03.29/////////////////////////////////////
	bool	bGiveUpGame; //이번판에 포기를 했는지 여부이다.
	//////////////////////////////////////////
};
...
//////////// NEWPOKER Add ToDo: 안쓰는 변수 제거하기
	...
	////////////////////////////골드시스템,골드채널 2017.11.07/////////////////////////////////////
	int ChipKind; //해당 방에서 어떤 재화로 게임을 했는지 알려준다.
	bool CommitionDiscount; //딜러비 할인을 하고 있는지 여부이다.true이면 할인중, false면 할인안함
	///////////////////////////////////////////
...
typedef struct  
{
	...
	////////////////////////////골드시스템,골드채널 2017.11.07/////////////////////////////////////
	int ChipKind; //게임을 진행하는 재화 타입이다. 위 CHIPKIND 에 정의되어 있음.. 새롭게 골드가 추가 되면서 필드도 새로 만들었다.
	//////////////////////////////////////////////
	bool bIsFriendRoom; // 친구룸 인가? // 2018년 5월 2일 랜덤매칭 시행령 관련 로직 추가
...
{
	//초대 및 가입신청관련 구조체이다.
	//내가 가입요청을 했던, 초대를 받았던 정보가 동일해서 같이 써도 무방
	//초대seq, 클럽ID, 클럽이름,회원수,최대회원수,상태코드,요청코드
	INT64 InviteSeq; //초대장 Sequence값
	int ClubID; //클럽ID
	char ClubName[CHAR_BUFFER_CLUB_NAME];
	int MemberMax; //가입할수 있는 총 회원수
	int MemberCount; //현재 가입된 회원수
	int StatusCode; //상태값.. 0.수락대기, 1.거절
	int RequestCode; //요청코드... 0.초대받은거 1.내가가입요청한거
...
typedef struct
{
	...
	INT64 PMoney; //보유머니
	INT64 LastLoginTime; //최근 접속시간(분단위로 계산하여 전송됨) - 가령 이 값이 55 로 가면 55분전에 접속했다는거.. 
} CLUB_MEMBER_INFO;
...
typedef struct
{
	...
	INT64 JoinSeq; //가입요청 Sequence값
	INT64 UserPMoney; //가입 유저의 보유머니
	int StatusCode; //상태값.. 0.수락대기, 1.거절
} CLUB_JOIN_INFO;
...
typedef struct
{
	//클럽 정보 구조체 이다.
	char ClubName[CHAR_BUFFER_CLUB_NAME];
	char BossID[NICK_LEN];   // 초대한사람 ID, 결국 보스가 초대를 했을것이므로..
	int ClubID; //클럽 고유번호
	int BossLevel; //보스 VIP 레벨
	INT64 BossPMoney; //보스가 보유중인 게임머니
	int MemberMax; //가입할수 있는 총 회원수
	int MemberCount; //현재 가입된 회원수
} CLUB_INFO;
////////////////////////////////////////
...
typedef struct
{
	//응모권으로 응모할 수 있는 상품 정보이다.
	char PM_Name[CHAR_BUFFER_PROMOTION_COMMON_PM_NAME];
	char GiftName[CHAR_BUFFER_PROMOTION_COMMON_TICKET_GIFT_NAME];
	int GiftIndex; //선물 고유번호(응모시 이 번호를 주면 됨)
	int NeedTicketCount; //필요한 응모권 갯수
} TICKET_GIFT_INFO;

typedef struct
{
	//티켓으로 응모한 정보
	int GiftIndex; //선물 고유번호
	int SB_Count; //응모한 횟수
...
typedef struct
{
	...
	INT64 JoinLimitMoney; //가입을 하기 위한 보유금액
	INT64 SaveMoney; //보험 가입시 적립하는 금액
