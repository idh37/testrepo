...
{ 
	...
	if ( lpszName == NULL )  
		bRetVal = FALSE;
	// If Drive Root
	else if ( (iDriveType = GetDriveType(lpszName)) != DRIVE_UNKNOWN 
		&& iDriveType != DRIVE_NO_ROOT_DIR )
		bRetVal = FALSE;
	else
	{	HANDLE handle;
...
// {
// 	ERROR_GAMEDIRECTGO error;
// 	BOOL bSucc = UPGGetSSOWebCookieString(strAuthCookie.GetBuffer(BUFFER_COOKIEDATA), strDataCookie.GetBuffer(BUFFER_COOKIEDATA), strCpCookie.GetBuffer(BUFFER_COOKIEDATA));
...
// 						else if(pFile->QueryInfoStatusCode(dwStatusCode)==FALSE 
// 							|| dwStatusCode != 200 )
...
// 	UPGNMRUNPARAMAPIFUNC_ENVVAR::GetEnvVar(NMRUNPARAMAPIFUNC_ENVVAR::ENVVARNAME_COMMONPATH, szExcuteFileName_Out.GetBuffer(MAX_PATH+1), MAX_PATH);
...
// 					UINT iRet;
// 
// 					if ( cRunParam.RunProgram(szExcuteFileName, &iRet) == ERROR_NMRUNPARAM_SUCCESS 
// 						&& iRet > 32 )
// 						error = ERROR_GAMEDIRECTGO_SUCCESS;
// 					else
