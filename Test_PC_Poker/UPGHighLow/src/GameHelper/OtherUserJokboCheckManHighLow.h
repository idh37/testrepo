...
typedef struct  
{
	...
	bool BubbleSort(){
		if( m_nTotalCardCount <= 0 )return false;

		int i=0, j=0, temp=-1,pic=-1;

		for(i = 0; i < m_nTotalCardCount-1; i++){
			if( m_listPossibleCard[i] == -1 )continue;
			for(j = i+1; j < m_nTotalCardCount; j++){
	...
	int get_Number( int index ){
		if( index < 0 || index >= m_nTotalCardCount )return -1;
...
typedef struct  
{
	...
	int get_TotalCardNum(){
		return m_CardInfo.m_nTotalCardCount;
	}
	...
	int get_PossibleCardNum( int index ){
		if( index < 0 || index >= m_CardInfo.m_nTotalCardCount )return -1;
...
class COtherUserJokboCheckManHighLow : public COtherUserJokboCheckMan
{
protected:
	bool				m_bUse;
	bool				m_bShow;
...
public:
	COtherUserJokboCheckManHighLow();
	virtual ~COtherUserJokboCheckManHighLow();
