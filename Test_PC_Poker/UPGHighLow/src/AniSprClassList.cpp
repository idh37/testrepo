...
//-----------------------(하이로우) (추가코드)-----------------------//	
//ANF_ASK_SELECTWINNERCASE,	//하이 , 로우 스윙 선택 하라,
...
void CSelectWinnerCaseEft_HI::Init(int pnum ,BOOL bAskSelect )
{
	...
	Destroy();
	m_nObjID = ANF_SELECTWINNERCASE_HI;	//종류		
	DrawFaceKind = DRAW_TOP;
	nDelayTime = 10;
	...
	//bAskSelect = TRUE		하이로우 선택 하라 sprno = 0
	//bAskSelect = FALSE	하이로우 선택을 완료 했다. sprno = 1
...
//ANF_RESULTSELECTCASE_HI,	//결과 처리 시 하이 , 로우 스윙 선택 효과,
...
void CResultSelectCaseEft_HI::Init( int pnum , EHIGHLOWSWINGCASE eResultCase )
{
	...
	Destroy();
	m_nObjID = ANF_RESULTSELECTCASE_HI;	//종류		
	DrawFaceKind = DRAW_BOTTOM;
	nDelayTime = 15;
