// PlayerHighLow.cpp: implementation of the PlayerHighLow class.
//
//////////////////////////////////////////////////////////////////////
...
void CPlayerHighLow::Clear()
{
	...
	m_eCaseSelect = CASE_NULL;
	m_eCaseResult = CASE_NULL;
}

void CPlayerHighLow::SetPlayerNum(int pnum)
{
	PNum = pnum;
}

void CPlayerHighLow::SetNewPlayer(USERINFO *pUI)
{	
	Clear();
...
//방에 입장할때 한번 초기화 하는것들 
void CPlayerHighLow::InitRoomData()
{
	Reset();
...
void CPlayerHighLow::OnSetChip(INT64 money)
{
...
void CPlayerHighLow::OnDraw_Bottom(CDC *pDC)
{
	// 버린카드 그리기
	OnDrawThrowCard(pDC);	
...
void CPlayerHighLow::OnDraw_Middle(CDC *pDC)
{
	m_cPlayerEffectList.MiddleDrawAll(pDC);
}

//각 m_cPlayer 최 상위에 들어갈 이미지들 
void CPlayerHighLow::OnDraw_Top(CDC *pDC)
{	
...
void CPlayerHighLow::OnCardDraw(NMBASE::GRAPHICGDI::CPage *pPage) 
{
...
BOOL CPlayerHighLow::IsCardFront(int index)
{
	return m_cMyCard[index].IsCardFront();
}

// 카드의 앞면을 보이게 한다.
void CPlayerHighLow::SetCardFrontShow(int index)
{
...
// 카드의 뒷면을 보이게 한다. 
void CPlayerHighLow::SetCardBackShow(int index)
{
	if(index < 0 || index >= m_nTotalCardCount) 
		return;	
...
void CPlayerHighLow::ShowCard(int index , BOOL bshow)
{
	if(index < 0 || index >= m_nTotalCardCount) 
		return;	
...
void CPlayerHighLow::SetResultFace(int index, BOOL bresult)
{
	if(index < 0 || index >= m_nTotalCardCount) 
		return;
...
// 0 ~ 6 총 7장
int CPlayerHighLow::GetCardNum(int index)
{
	if(index < 0 || index >= m_nTotalCardCount) 
		return 0;
...
// 카드를 다 덮은 상태로 모은다.
void CPlayerHighLow::SetGameResultCardOpen(int delay)	// 카드 오픈 (애니 처리 설정)
{
...
void CPlayerHighLow::SetChioceCardMove(int face)
{
...
// 카드 영역을 클릭 했는지 체크
BOOL CPlayerHighLow::IsGameOverPtInCardRect(int xp,int yp) 
{
	// 다이 일때 내카드 열기
...
// 카드 영역에 있는지
int CPlayerHighLow::EventOverPtInCardRect(CPoint pt)
{
	for(int i = 0 ; i < m_nTotalCardCount ; ++i)
	{
...
void CPlayerHighLow::OnDrawThrowCard(CDC *pDC)
{
...
// 게임키 처리를 한다.
void CPlayerHighLow::GameKeyProcess(int nVKey, int nExtend)
{
...
// 								AddGameChatViewEdit(szTemp.GetBuffer(), GetChatColor(ECC2_RESULT_JOKBO));
	...
	//하이로우 전용 (하이/로우/스윙 선택)
	if( g_RI.State == EGAME_PLAYING &&  g_Poker.nState == RSTATE_HIGHLOW_SELECT && JoinState == EGAME_JOININ && PlayState == EPLAYER_GAMEPLAYING && PNum == 0)
	{

		for( int i = KEY_HILOWSEL_HIGH; i <= KEY_HILOWSEL_SWING; ++i )
		{
			for( int j = 0; j < MAX_GAME_KEY; ++j )
			{				
				if( IS_VKEY( G_KEYDEF[i][j], nVKey, nExtend ) )
				{
					...
					switch( i )
					{
					case KEY_HILOWSEL_HIGH:
						{
							g_GAMEPROCESS()->SendHighCase();
						} break;
					case KEY_HILOWSEL_LOW:
						{
							g_GAMEPROCESS()->SendLowCase();
						} break;
					case KEY_HILOWSEL_SWING:
						{
							g_GAMEPROCESS()->SendSwingCase();
						} break;
...
void CPlayerHighLow::OnLButtonDown(int xp, int yp)
{
	if(GetCurrentWhere() != IDX_GAMEWHERE_GAME) return;
...
void CPlayerHighLow::OnLButtonUp(int xp, int yp)
{
	if( GetCurrentWhere() != IDX_GAMEWHERE_GAME ) return;
...
void CPlayerHighLow::OnMouseMove(int xp, int yp)
{
	if(GetCurrentWhere() != IDX_GAMEWHERE_GAME)
		return;
...
int CPlayerHighLow::GetHiddenCardMoveWay( int xp, int yp )
{
...
void CPlayerHighLow::HiddenOpne()
{
...
//이팩트 효과 지우기
void CPlayerHighLow::ClearEffectSpr(ENUM_ANIFILEID kind, BOOL ballclear)
{
	if( ballclear == TRUE )
	{
		m_cPlayerEffectList.Destroy();
...
void CPlayerHighLow::SetCardResult()
{	
	int nCardNum[10] = {0, };
...
//카드 배열 정렬 기본형,일자형
void CPlayerHighLow::set_CardArray()
{	
	if(GetCurrentWhere() != IDX_GAMEWHERE_GAME || g_ObserverMan.IsMyObserver() || g_RI.State == EGAME_WAITING || PNum != 0 ) 
		return;
...
// 히든카드를 보여준다.
void CPlayerHighLow::DrawDummyHiddenCard(CDC *pDC)
{
	if( GetCurrentWhere() != IDX_GAMEWHERE_GAME ) return;
...
// 화면에 카드를 추가 하고 세팅한다.
void CPlayerHighLow::SetAddCard( int nCardNum )
{	
...
void CPlayerHighLow::Draw_DarkCard()
{
	if( JoinState == EGAME_JOINOUT )
	{
		return;
	}

	if (PNum<0 || PNum>=g_GAMEPROCESS()->GetMaxGamePlayer())
	{
		return;
	}
	
	int i = 0;
	if ( m_eCaseResult == CASE_HIGH ){
		for(i = 0 ; i < m_nTotalCardCount ; i++ ){
	...
	else if ( m_eCaseResult == CASE_LOW )
	{
		for(i = 0 ; i < m_nTotalCardCount ; i++ ){
			if( g_GAMEPROCESS()->GetSevenRuleManager().m_cRule[PNum].m_RemainCard_Low[i] > -1 ){
				int num = g_GAMEPROCESS()->GetSevenRuleManager().m_cRule[PNum].m_RemainCard_Low[i];
				for(int j = 0; j < m_nTotalCardCount ; j ++ ){
					if( m_cMyCard[j].GetCardNum() == num ){
						m_cMyCard[j].SetResultDarkDraw(TRUE);
						break;
	...
	else if( m_eCaseResult == CASE_SWING || bFoldOpen ){
		for(i = 0 ; i < m_nTotalCardCount ; i++ ){		
			m_cMyCard[i].SetResultDarkDraw(TRUE);			
		}
		//하이 
		for(i = 0 ; i < m_nTotalCardCount ; i++ ){
			if( g_GAMEPROCESS()->GetSevenRuleManager().m_cRule[PNum].m_RuleComplteCard[i] > -1 ){
				int num = g_GAMEPROCESS()->GetSevenRuleManager().m_cRule[PNum].m_RuleComplteCard[i];
				for(int j = 0; j < m_nTotalCardCount ; j ++ ){
					if( m_cMyCard[j].GetCardNum() == num ){
						m_cMyCard[j].SetResultDarkDraw(FALSE);
						break;
					}
				}
			}
		...
		//로우
		for(i = 0 ; i < m_nTotalCardCount ; i++ ){
			if( g_GAMEPROCESS()->GetSevenRuleManager().m_cRule[PNum].m_RuleComplteCard_Low[i] > -1 ){
				int num = g_GAMEPROCESS()->GetSevenRuleManager().m_cRule[PNum].m_RuleComplteCard_Low[i];
				for(int j = 0; j < m_nTotalCardCount ; j ++ ){
					if( m_cMyCard[j].GetCardNum() == num ){
						m_cMyCard[j].SetResultDarkDraw(FALSE);
						break;
...
BOOL CPlayerHighLow::OnGameKey(int nVKey, int nExtend, int repeat)
{	
	if(GetCurrentWhere() != IDX_GAMEWHERE_GAME)
	{
		return FALSE;
...
void CPlayerHighLow::DrawEft_CompleteMadeValue(int oldpoint, bool bForce )
{
...
INT64 CPlayerHighLow::GetNowMoney()
{
	if( g_RI.bIsMaxRoom )
	{
...
void CPlayerHighLow::SetNowMoney( INT64 nMoney )
{
	if( g_RI.bIsMaxRoom )
	{
