...
namespace NMBASE
{
	namespace GRAPHICGDI
	{
		...
		void xAniFile::Destroy()
		{
			if(ppCharacter) 
			{
				for(int i=0; i<TotCharacterNum; i++) 
					if(ppCharacter[i] != NULL) { 
						SafeDelete<Character>(&ppCharacter[i]);//delete(ppCharacter[i]); 
						ppCharacter[i]=NULL; 
					}
					free(ppCharacter);
					ppCharacter= NULL;
		...
		void xAniFile::AddCharacter(char* name)
		{
			...
			if(ppCharacter) 
			{
				memcpy(ppNew, ppCharacter, sizeof(Character*) * (TotCharacterNum-1));
				free(ppCharacter);
			}
			ppCharacter = ppNew;
			...
			if(ppCharacter)
			{
				...
				if(ppCharacter[TotCharacterNum-1])
				{
		...
		void xAniFile::DelCharacter(int num)
		{
			...
			SafeDelete<Character>(&ppCharacter[num]);
		...
		void Character::AddGroup(char* name)
		{
			...
			if(ppGroup)
			{
				memcpy(ppNew, ppGroup, sizeof(Group*) * (TotGroupNum-1));
				free(ppGroup);
			}
			ppGroup = ppNew;
		...
		void Group::AddAction(char* name)
		{
			...
			if(ppAction)
			{
				memcpy(ppNew, ppAction, sizeof(Action*) * (TotActionNum-1));
				free(ppAction);
			}
			ppAction = ppNew;
		...
		void Group::DelAction(int num)
		{
			...
			//delete(ppAction[num]);
			SafeDelete<Action>(&ppAction[num]);
		...
		void Frame::AddCell(int sprno, int rx, int ry, int overrideflag)
		{
			...
			if(ppCell)
			{
				memcpy(ppNew, ppCell, sizeof(Cell*) * (TotCellNum-1));
				free(ppCell);
			}
			ppCell = ppNew;
