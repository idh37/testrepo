...
namespace NMBASE
{
	namespace GRAPHICGDI
	{
		...
		char xhan_outline_temp1[128], xhan_outline_temp2[128];	// 아웃라인 생성용 임시 버퍼
		...
		*/
			...
			WORD *tar, int bgr, WORD bgrColor, int type, WORD color)
		{
			...
			if(type==2)  // 외곽선출력
			{
				...
				memcpy(xhan_outline_temp1, buf, size);
				memcpy(xhan_outline_temp2, buf, size);

				for(i=0; i<size; i++)
				{
					xhan_outline_temp1[i] |= xhan_outline_temp2[i]>>1;
					xhan_outline_temp1[i] |= xhan_outline_temp2[i]<<1;
				}

				for(i=0; i<size-xsize; i++)
				{
					xhan_outline_temp1[i+xsize] |= xhan_outline_temp2[i];
				}
				...
				{
					xhan_outline_temp1[i] ^= xhan_outline_temp2[i];
				}
				...
				for(i = 0; i < yl; i++)
				{
					...
					for(j=0; j<xl; j++)
					{
						if((xhan_outline_temp1[m] & (mask >> n)) == (mask >> n)) *tar = color;
						else if(bgr) *tar=bgrColor;
						n++;
						if(n == 8) if(j+1 != xl) { n = 0; m++; }
						tar++;
