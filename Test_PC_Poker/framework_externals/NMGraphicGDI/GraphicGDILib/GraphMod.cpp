...
#define LERP(alpha, v1, v2) ((v1)+(alpha)*((v2)-(v1)))
		...
		void (_fastcall *MemCpyGraysclAB) (WORD *tar, WORD *sor, int Len, BYTE* palpha, int grade);
		...
		BOOL InitGrpLib(int xl, int yl, DWORD lPitch, int PixelFormat)
		{
			...
			if(PixelFormat==565)
			{
				...
				MemCpyGraysclAB = MemCpyGrayscl565AB;
			...
			else
			{
				...
				MemCpyGraysclAB = MemCpyGrayscl555AB;
			...
			for( int i = 0 ; i < 360 ; i++ )
			{
				CosTable[i] = cos(i* 3.14/180);
				SinTable[i] = sin(i* 3.14/180);
			}
			return TRUE;
		}
		...
		void _PutImage(int xp, int yp, int xLen, int yLen, WORD *tar, WORD *sor )
		{
			...
			if( xp > _CX2 || yp > _CY2 || xp+xLen <= _CX1 || yp+yLen <= _CY1 ) 
				return;
		...
		void _PutSprLR(int xp, int yp, int xLen, int yLen, WORD *tar, WORD *sor, WORD colorkey)
		{
			int x=0, y=0, h=0, t=0;  // 초과치
			int i,j;
		...
		void _PutSpr_Mono(int xp, int yp, int xLen, int yLen, WORD *tar, WORD *sor, WORD colorkey, WORD color, int grade)
		{
			int x=0, y=0, h=0, t=0;  // 초과치
			int i,j;
		...
		void _PutSpr_Mono_AB(int xp, int yp, int xLen, int yLen, WORD *tar, WORD *sor, BYTE* alpha_table, WORD color, int grade)
		{
			int x=0, y=0, h=0, t=0;  // 초과치
			int i,j;
			...
			for(i=0; i<yLimit; ++i) {
				BYTE* palpha = alpha_table + (i * xLen) + x;
				for(j=0; j<xLimit; ++j) {
					int alpha = *palpha;
		...
		void _PutSpr_Grayscl(int xp, int yp, int xLen, int yLen, WORD *tar, WORD *sor, int grade)
		{
			int x=0, y=0, h=0, t=0;  // 초과치
			int i,j;
				...
				for(j=0; j<xLimit; ++j) {
					//	if(*sor != colorkey) 
					{
						if( grade != 31)
						{
							...
							int dest_alpha = 31-alpha;
							if(alpha > 0) {
								WORD targetcolor = *tar;
								MemCpyGrayscl(&targetcolor,sor,1);
								*tar = ALPHAMIXW_555(targetcolor, targetcolor, dest_alpha, alpha);
							}
						}
						else
						{
							MemCpyGrayscl(tar,sor,1);
						}
		...
		void _PutSpr_Grayscl_AB(int xp, int yp, int xLen, int yLen, WORD *tar, WORD *sor, BYTE* alpha_table, int grade)
		{
			int x=0, y=0, h=0, t=0;  // 초과치
			int i,j;
		...
		/*
		void _fastcall RvrsCpyAB(WORD *tar, WORD *sor, int Len)
		{
		int i;
		tar += Len-1;
		for(i=0; i<Len; i++) *tar-- = *sor++;
		...
		*/
		...
		void _fastcall MemCpyHB555(WORD *tar, WORD *sor, int Len)
		{
			...
			for(int i = 0; i < HalfLen; ++i, ++p4_scr, ++p4_spr)
			{
		...
		void _fastcall MemCpyGrayscl555AB(WORD *tar, WORD *sor, int Len,BYTE* pAlpha,int grade)
		{
			int i;
			WORD val;
			for(i=0; i<Len; ++i, ++tar, ++sor,++pAlpha) {
				int alpha = ABGRADE[grade][(*pAlpha)];
				int dest_alpha = 32-alpha;
				if(alpha > 0){
					//val = (R555(*sor)>>1) + (G555(*sor)>>1) + (B555(*sor)>>1);	// RGB 각 원소별 평균값(약간 오차가 생기나 속도가 빠름)
					val = ALPHAMIXW_555(*tar , *sor  , dest_alpha, alpha);
					val = (R555(val) + G555(val) + B555(val)) / 3;			// RGB 각 원소별 평균값(정확한 산출식)
					*tar = (val<<10) | (val<<5) | (val);
				}
			}
		}

		void _fastcall MemCpyGrayscl565AB(WORD *tar, WORD *sor, int Len,BYTE* pAlpha,int grade)
		{
			int i;
			WORD val;
			for(i=0; i<Len; ++i, ++tar, ++sor,++pAlpha) {
				...
				if(alpha > 0) {
					// 아직 미완성(사용 불가)
					val = (R[R565(*sor)][alpha] | G[G565(*sor)][alpha] | B[B565(*sor)][alpha]) +
						(R[R565(*tar)][dest_alpha] | G[G565(*tar)][dest_alpha] | B[B565(*tar)][dest_alpha]);
				}
				//	val = ALPHAMIXW_555(*tar , *sor  , dest_alpha, alpha);
				val = (R565(val) + (G565(val)>>1) + B565(val)) / 3;	// RGB 각 원소별 평균값(정확한 산출식)
				*tar = (val<<11) | (val<<6) | (val);
		...
		void _PutCsp_Grayscl_AB(int xp, int yp, int xLen, int yLen, WORD *tar, WORD *sor, BYTE* alpha_table, int grade )
		{
			int x=0, y=0, h=0, t=0;  // 초과치
			...
			if( x || h )               // 클리핑이 발생되면
			{
				...
				for(i=0;i<yLimit;i++,tar+=_LPITCH)
				{
					...
					sbyte=0;
					doing=0;
					BYTE* palpha = alpha_table + xLen*i + x;
					while(cnt--)         // 패턴수 만큼 반복
					{
						sbyte += *sor++;
						pbyte = *sor++;
						...
						if(doing==BEGINPUTSPR)   // 출력할 상황이면
						{
							...
							{
								MemCpyGraysclAB(tar+sbyte, sor, (xLimit-sbyte),palpha,grade);
								doing = ENDPUTSPR;
							}
							else MemCpyGraysclAB(tar+sbyte, sor, pbyte,palpha,grade);
						}
			...
			else  // 클리핑이 발생되지 않았거나 위쪽과 아래쪽만 발생한 경우
			{
				...
				for(i=0; i<yLimit; i++,tar+=_LPITCH)
				{
					...
					while(cnt--)         // 패턴수 만큼 반복
					{
						...
						pbyte = *sor++;  // 이미지 길이(점의 개수)
						palpha+=sbyte;
						MemCpyGraysclAB(tar+sbyte, sor, pbyte,palpha,grade);
		...
		void _fastcall MemSetAB(WORD *tar, WORD color, BYTE* palpha_table , int Len)
		{
			int i;
			BYTE*  palpha = palpha_table;
			for(i=0; i<Len; i++) 
			{
				int alpha = *palpha;
				int dest_alpha = 32-alpha;
		...
		void _fastcall MemSetGradeAB(WORD *tar, WORD color, BYTE* palpha_table , int Len,int grade)
		{
			int i;
			BYTE*  palpha = palpha_table;
			for(i=0; i<Len; i++) 
		...
		void _PutCsp_Mono_AB(int xp, int yp, int xLen, int yLen, WORD *tar, WORD *sor, BYTE* alpha_table, WORD color,int grade)
		{
			int x=0, y=0, h=0, t=0;  // 초과치
				...
				for(i=0;i<yLimit;i++,tar+=_LPITCH)
				{
					...
					while(cnt--)         // 패턴수 만큼 반복
					{
						...
						if(doing==BEGINPUTSPR)   // 출력할 상황이면
						{
							...
							{
								palpha+= sbyte;
								MemSetGradeAB(tar+sbyte, color,palpha, (xLimit-sbyte),grade);
								doing = ENDPUTSPR;
							}
							else 
							{
								palpha+= sbyte;
								MemSetGradeAB(tar+sbyte, color,palpha ,pbyte,grade);
							}
						}
		...
		void _fastcall MemCpyGB555(WORD *tar, WORD *sor, int Len, int alpha)
		{
			...
			for(i=0; i<HalfLen; ++i)
			{
				...
				*p4_scr = (
					(((( (*p4_scr) & d4AM_RGB555_1) >> 5 ) * dest_alpha ) & d4AM_RGB555_1 ) +
					(((( (*p4_spr) & d4AM_RGB555_1) >> 5 ) * alpha      ) & d4AM_RGB555_1 ) 	
					) 
					|
					(
					(((( (*p4_scr) & d4AM_RGB555_2) * dest_alpha ) >> 5 ) & d4AM_RGB555_2 ) + 
		...
		void _fastcall MemCpyAB555(WORD *tar, WORD *sor, int Len, BYTE* palpha, int grade)
		{
			for(int i=0; i<Len; ++i)
			{
				...
				if(alpha > 0) {
					...
					//	if(alpha == 31) *tar = *sor;
					//	else *tar =  (R[R555(*sor)][alpha] | G[G555(*sor)][alpha] | B[B555(*sor)][alpha]) +
					//			   (R[R555(*tar)][dest_alpha] | G[G555(*tar)][dest_alpha] | B[B555(*tar)][dest_alpha]);
		...
		inline void _MemCpyAB555(WORD *tar, WORD *sor, int Len, BYTE* palpha, int grade)
		{
			for(int i=0; i<Len; ++i)
			{
				int alpha = ABGRADE[grade][(*palpha)];
		...
		void _PutCspLR_AB(int xp, int yp, int xLen, int yLen, WORD* tar, WORD* sor, BYTE* alpha_table, int grade)
		{
			...
			if( x || h )         // 좌우 클리핑이 발생되면
			{
				...
				for(i=0;i<yLimit;i++,tar+=_LPITCH)
				{
					...
					sbyte=0;
					doing=0;
					BYTE* palpha = alpha_table + (xLen * (y + i))+h;
					while(cnt--)         // 패턴수 만큼 반복
					{
						sbyte += *sor++;
						pbyte =  *sor++;
			...
			else    // 클리핑이 발생되지 않았거나 위쪽과 아래쪽만 발생한 경우
			{
				...
				for(i=0; i<yLimit; i++,tar+=_LPITCH)
				{
					BYTE* palpha = alpha_table + (xLen * (y+i));
					cnt = *sor++;
					sbyte=0;
					while(cnt--)         // 패턴수 만큼 반복
					{
		...
		void _PutCspUD_AB(int xp, int yp, int xLen, int yLen, WORD *tar, WORD *sor, BYTE* alpha_table, int grade)
		{
			...
			if( x || h ||y || t)         // 좌우 클리핑이 발생되면
			{
				...
				for(i=0;i<yLimit;i++,tar-=_LPITCH)
				{
					...
					sbyte=0;
					doing=0;
					BYTE* palpha = alpha_table + (xLen * (i+t)) + x;
					while(cnt--)         // 패턴수 만큼 반복
					{
						sbyte += *sor++;
						pbyte = *sor++;
			...
			else    // 클리핑이 발생되지 않았거나 위쪽과 아래쪽만 발생한 경우
			{
				...
				{
					cnt = *sor++;
					BYTE* palpha = alpha_table + (xLen * i);
					sbyte=0;
					while(cnt--)         // 패턴수 만큼 반복
					{
						sbyte += *sor++;
