...
static AFX_EXTENSION_MODULE NMModelDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("NMModel.DLL을 초기화하고 있습니다.\n");
		
		// 확장 DLL을 한 번만 초기화합니다.
		if(!AfxInitExtensionModule(NMModelDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(NMModelDLL);

		// Model의 Old-Default 네트웍 객체를 생성하기 위한 호출
		NMBASE::MODEL::GNET().Init();

		// Model의 Camel 네트웍 객체를 생성하기 위한 호출
		NMBASE::MODEL::GCAMELNET().Init();
	}
	else if(dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("NMModel.DLL을 종료하고 있습니다.\n");
		...
		AfxTermExtensionModule(NMModelDLL);
	}
	return 1;   // 확인
}
