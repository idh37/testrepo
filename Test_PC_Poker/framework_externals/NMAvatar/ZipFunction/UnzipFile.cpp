...
bool CUnzipFile::LocateFile(CString szFileName, bool bCaseSensitive)
{
	...
	while (res)
	{
		char szCurrentFileName[UNZ_MAXFILENAMEINZIP+1] = {0, };
		GetCurrentFileInfo(NULL, szCurrentFileName, sizeof(szCurrentFileName)-1, NULL,0,NULL,0);
		CString szTemp;
		szTemp.Format("%s", szCurrentFileName);

		if(StringFileNameCompare(szTemp, szFileName, bCaseSensitive)==0)
			return true;
