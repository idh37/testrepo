...
namespace NMBASE
{
	namespace AVATAR
	{
		...
		BOOL OpenLoadingSprite(xSprite *pSprite)
		{
			...
			// Afx가 붙는 LoadLibrary, FreeLibrary를 쓴 이유는 이미지파일을 담은 DLL이 MFC확장 DLL이기 때문입니다. 
			// 일반 DLL이면, LoadLibrary, FreeLibrary 쓰면 되겠죠
		...
		BOOL Init(LPCTSTR strBaseDir)
		{
			...
			{
				::MessageBox(NULL, "아바타 모듈을 초기화할 수 없습니다.\n(아바타 모듈 초기화 실패)", "Error", MB_OK|MB_ICONERROR);
				return FALSE;
			}
		...
		BOOL Init()
		{
			...
			{
				::MessageBox(NULL, "아바타 모듈을 초기화할 수 없습니다.\n(레지스트리 값을 찾을 수 없습니다.)", "Error", MB_OK|MB_ICONERROR);
				return FALSE;
			}	
		...
		BOOL CAvatar::SetAvatar(short *pAvatarInfo)
		{
			...
			// 25번 레이어는 DLL에서 강제로 정상출력하기 때문에 필요 없음

			/*
			//
			// [리더스 아이콘 코드 - 기존]
			// 1: 리더스 2:리더스 플러스 3:리더스(반전) 4:리더스 플러스(반전)
			// 5: 리더스 미니 6: 리더스 VIP 7: 리더스 미니(반전) 8: 리더스 VIP(반전)
			//

			//
			// [리더스 아이콘 코드 - NEW]
			// 10: 리더스 미니 12: 리더스 14:리더스 플러스 : 16:리더스 VIP
			// 반전은 -1
			//
			...
			// 리더스 아이콘이 뒤집혀서 찍힐 경우의 이미지 번호를 설정
		...
		BOOL CAvatar::raw_SetAvatar(HAVATAR h, int *pLayerItem)
		{
			...
			if(item[LN_FACE] <= 0) 
				item[LN_FACE] = ((defitem==1)?(1):(6));	// 표정은 5개씩 그룹임
