...
BOOL CPictureEx::Load(HGLOBAL hGlobal, DWORD dwSize)
{
		...
		(memcmp(&m_pGIFHeader->m_cVersion,"89a",3) != 0)) )
	{
		...
		*/
		...
		// 그림 사이즈 저장
		m_PictureSize.cx = hmWidth;
		m_PictureSize.cy = hmHeight;
	}
	else
	{
		//
		...
		if(m_pGIFLSDescriptor->GetPackedValue(LSD_PACKED_GLOBALCT) == 1)
		{
			...
			unsigned char *pBkClr = m_pRawData + sizeof(TGIFHeader) + 
				sizeof(TGIFLSDescriptor) + 3*m_pGIFLSDescriptor->m_cBkIndex;	// 컬러키
...
int	CPictureEx::GetFrameDelay(int frame) 
{ 
	if(!IsGIF())
		return 0;

	if(frame < 0 || frame >= (int)m_arrFrames.size()) 
		return 0;

	return m_arrFrames[frame].m_nDelay;
}

int CPictureEx::GetFrameDisposal(int frame) 
{
	if(!IsGIF())
		return 0;
	...
	return m_arrFrames[frame].m_nDisposal;
}
...
BOOL CPictureEx::LoadFromMemory(void* pImage, DWORD dwSize)
{
	ASSERT(pImage);
...
BOOL CPictureEx::DrawSingleFrameToDC(HDC hdc, int xp, int yp, int frame)
{
	if(!m_bIsInitialized)
	{
		return FALSE;
	...
	if(IsAnimatedGIF())
	{
		int totframe = m_arrFrames.size();
		if(totframe == 0 || frame < 0 || frame >= totframe) 
			return FALSE;

		long hmWidth;
		long hmHeight;
		m_arrFrames[frame].m_pPicture->get_Width(&hmWidth);
		m_arrFrames[frame].m_pPicture->get_Height(&hmHeight);
		...
		*/
		...
		int x = xp + m_arrFrames[frame].m_frameOffset.cx;
		int y = yp + m_arrFrames[frame].m_frameOffset.cy;

		if(m_arrFrames[frame].m_pPicture->Render(hdc, x, y, 
			m_arrFrames[frame].m_frameSize.cx, m_arrFrames[frame].m_frameSize.cy, 
			0, hmHeight, hmWidth, -hmHeight, NULL) == S_OK)
		{
			//Invalidate(FALSE);
			return TRUE;
	...
	else 
	{
		...
		m_pPicture->get_Width(&hmWidth);
		m_pPicture->get_Height(&hmHeight);
		if(m_pPicture->Render(hdc, xp, yp, m_PictureSize.cx, m_PictureSize.cy, 0, hmHeight, hmWidth, -hmHeight, NULL) == S_OK)
		{
			//Invalidate(FALSE);
			return TRUE;
		}
...
int CPictureEx::GetNextBlockLen() const
{
	...
	case BLOCK_IMAGE:
		...
		int nLCTSize = (int)(pIDescr->GetPackedValue(ID_PACKED_LOCALCT) * 3 * (1 << (pIDescr->GetPackedValue(ID_PACKED_LOCALCTSIZE)+1)));
...
void CPictureEx::EnumGIFBlocks()
{
	...
	while(m_nCurrOffset < m_nDataSize)
	{
		...
		case BLOCK_IMAGE:
			TGIFImageDescriptor *pIDescr = reinterpret_cast<TGIFImageDescriptor *> (&m_pRawData[m_nCurrOffset]);
			TRACE(_T("- Image data block (%dx%d  %d,%d)\n"), pIDescr->m_wWidth, pIDescr->m_wHeight, pIDescr->m_wLeftPos, pIDescr->m_wTopPos);
			break;
		};
