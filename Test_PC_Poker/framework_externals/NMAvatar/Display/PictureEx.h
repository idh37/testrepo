...
public:
	...
	// 메모리에서부터 GIF를 읽어들임
	BOOL LoadFromMemory(void* pImage, DWORD dwSize);

	BOOL DrawSingleFrameToDC(HDC hdc, int xp, int yp, int frame);

	int	GetFrameDelay(int frame);

	int GetFrameDisposal(int frame); 
...
public:
	...
	HANDLE m_hThread;
	HANDLE m_hExitEvent;
	IPicture * m_pPicture;						// 일반 이미지의 이미지 인터페이스
	TGIFHeader * m_pGIFHeader;
	unsigned char * m_pRawData;
	TGIFLSDescriptor * m_pGIFLSDescriptor;
	vector<TFrame> m_arrFrames;			// 에니메이션 GIF의 프레임 배열
