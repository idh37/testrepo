...
#define MCIMEDIA_SUCCESS   (0) // MCI 함수리턴값중 성공시 0을 리턴하는 경우가 많다
		...
		{
			if(GetStatus(MCI_STATUS_MODE) == MCI_MODE_PAUSE)
				return TRUE;
			return FALSE;
		}

		BOOL CMciMedia::IsPlaying()
		{
			if(GetStatus(MCI_STATUS_MODE) == MCI_MODE_PLAY)
				return TRUE;
			return FALSE;
		}
		...
		{
			if(GetStatus(MCI_STATUS_MODE) == MCI_MODE_STOP)
				return TRUE;
			return FALSE;
		}
		...
		{
			if(GetStatus(MCI_STATUS_MODE) == MCI_MODE_RECORD)
				return TRUE;
			return FALSE;
		}
		...
		void CMciMedia::SetVolume(DWORD dwVolume)
		{
			...
			dwVolume = (DWORD)((dwVolume * (0xFFFF / 100.0f)) * (xSound::GetMasterVolume()/100.0f));
