...
namespace NMBASE
{
	namespace SOUND
	{
		...
		HRESULT xSoundBuffer::Create(char* filename, int count)
		{
			...
			rval = xSound::GetDS()->CreateSoundBuffer(&desc, &m_Buffer[0], NULL);
			if(rval != DS_OK) 
			{
				goto CREATE_ERROR;
			}
			...
			{
				rval = xSound::m_DirectSound->DuplicateSoundBuffer(m_Buffer[0], &m_Buffer[i]);
			}
...
CREATE_ERROR:
		...
		BOOL xSoundBuffer::IsPlaying(int channel/*=-1*/)
		{
			...
			if(channel < 0) 
			{
				for(int i = 0; i < m_BufferCount; ++i)
				{
					...
					if(m_Buffer[i]->GetStatus(&Status) != DS_OK) 
					{
						continue;
					}
			...
			else
			{
				...
				if(m_Buffer[channel] == NULL) 
				{
					return FALSE;
				}

				if(m_Buffer[channel]->GetStatus(&Status) != DS_OK) 
				{
					return FALSE;
				}
		...
		// xSoundBuffer Stop
		HRESULT xSoundBuffer::Stop(int channel)
		{
			HRESULT rval;

			if(!xSound::IsInitDSound || !m_Buffer) 
			{
				return E_FAIL;
			}
			...
			else 
			{
				...
				rval = m_Buffer[channel]->Stop();
				if(rval != DS_OK) 
				{
					return rval;
				}

				rval = m_Buffer[channel]->SetCurrentPosition(0);
				if(rval != DS_OK) 
				{
					return rval;
				}
		...
		/*
		// 음량 설정이 세밀하게 되지 않는 문제를 수정 하기 위해 함수 교체 [10/11/2011 guesty]
		...
		BOOL xSoundBuffer::SetSoundVolume(LONG lChannelNum, LONG lVolume)
		{
			...
			lVolume = (LONG)((lVolume * 100) * (xSound::GetMasterVolume()/100.0f)) - 10000 ;		// -10000 ~ 0
			...
			if(lChannelNum >= m_BufferCount)
			{
				return FALSE;
			}

			if(m_Buffer[lChannelNum] == NULL)
			{
				return FALSE;
			}
		...
		*/
		...
		HRESULT x3DSoundBuffer::Create(char* filename, int count)
		{
			...
			// Create enough space for the buffers
			m_BufferCount = max(1, count);
			m_Buffer = new LPDIRECTSOUNDBUFFER[m_BufferCount * sizeof(LPDIRECTSOUNDBUFFER)];
			if(m_Buffer == NULL)
				return E_FAIL;
			...
			desc.dwFlags = DSBCAPS_CTRL3D|DSBCAPS_CTRLPAN|DSBCAPS_CTRLVOLUME|DSBCAPS_CTRLFREQUENCY|DSBCAPS_GLOBALFOCUS;
...
CREATE_ERROR:
		...
		BOOL xStream::Create(LPSTR pszFilename)
		{
			...
			if(!xSound::IsInitDSound) return FALSE;
			...
			if(pszFilename)
			{
				...
				if(m_pwavefile = new xWaveFile)
				{
					...
					if(m_pwavefile->Open(pszFilename))
					{
						...
						m_dsbd.dwFlags = DSBCAPS_GETCURRENTPOSITION2|DSBCAPS_CTRLPAN|DSBCAPS_CTRLVOLUME|DSBCAPS_CTRLFREQUENCY|DSBCAPS_GLOBALFOCUS;
		...
		BOOL xStream::SetVolume(LONG lVolume)
		{
			...
			lVolume = (lVolume * xSound::GetMasterVolume()) - 10000;		// -10000 ~ 0
		...
		BOOL xStream::TimerCallback(DWORD dwUser)
		{
			...
			xStream* pStream = (xStream*) dwUser;

			return pStream->ServiceBuffer();
		}
		...
		void CALLBACK xMMTimer::TimeProc(UINT uID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2)
		{
			...
			if(IsBadReadPtr(ptimer, sizeof(xMMTimer)))
				return;
		...
		BOOL xWaveFile::Open(LPSTR pszFilename)
		{
			...
			if((m_mmckiRiff.ckid != FOURCC_RIFF) || ( m_mmckiRiff.fccType != mmioFOURCC('N', 'M', 'V', 'E') && m_mmckiRiff.fccType != mmioFOURCC('W', 'A', 'V', 'E') ) )
			{
...
READ_DONE:
		...
		{
			if(wFlags==MCI_NOTIFY_SUCCESSFUL && m_isLoop==TRUE)
				Replay();
		}
...
#define MAX_CD_TRACKS 50
		...
		short xMusicCD::Read(void)
		{
			...
			if(m_nNumberOfTracks > MAX_CD_TRACKS)
				m_nNumberOfTracks = MAX_CD_TRACKS;
