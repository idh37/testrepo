...
#define ALPHAMIXW_555(tarw, srcw, talpha, salpha) \
	(\
	(R[R555(srcw)][salpha] | G[G555(srcw)][salpha] | B[B555(srcw)][salpha])+ (R[R555(tarw)][talpha] | G[G555(tarw)][talpha] | B[B555(tarw)][talpha])\
	)
		// + (R[R555(tarw)][talpha] | G[G555(tarw)][talpha] | B[B555(tarw)][talpha])
		...
		*/
		...
		extern	int		ABGRADE[32][32];				// 알파 블렌딩 그레이드 테이블
		extern	int     RVS_ABGRADE[32][32];			// 리버스 알파 블렌딩 그레이드 테이블 (Darken Effect 색상 변환등에 사용)
		...
		// 그레이 스케일 효과로 메모리를 순차 복사(주의: WORD단위 복사, Len=WORD길이)
		extern void _fastcall MemCpyGrayscl555AB(WORD *tar, WORD *sor, int Len, BYTE* palpha,int grade);
		extern void _fastcall MemCpyGrayscl565AB(WORD *tar, WORD *sor, int Len, BYTE* palpha,int grade);
		...
		// 알파블렌딩으로  메모리를 역순 복사(주의: WORD단위 복사, Len=WORD길이)
		...
		extern void (_fastcall *MemCpyGraysclAB) (WORD *tar, WORD *sor, int Len, BYTE* palpha, int grade);
