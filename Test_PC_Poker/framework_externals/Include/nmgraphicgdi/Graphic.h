...
namespace NMBASE
{
	namespace GRAPHICGDI
	{
		...
		/// 그래픽 출력용 CPage 클래스
		class NMGRAPHICGDI_CLASS CPage
		{
		public:
			CPage();
			~CPage();
		...
		public:
			...
			BOOL PutSprAuto(int x, int y, CPage *pPage, int opt=NULL, int grade=31 , WORD color=0);
			...
			BOOL ZoomSprAuto(int x, int y, CPage *pPage, int tx, int ty, int opt=NULL, int grade=31, WORD color=0);
			...
			BOOL RotateZoomSprAuto(int x, int y, CPage *pPage, int tx, int ty, float angle, int opt=NULL, int grade=31, WORD color=0);
		...
		NMGRAPHICGDI_CLASS WORD GetRGBmix(int nR, int nG, int nB);
