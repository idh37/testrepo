// NMNMRunParamModuleLib.h: interface for the NMRunParamModuleLib class.
//
//////////////////////////////////////////////////////////////////////
#pragma once
...
namespace NMBASE
{
	namespace UTIL
	{
		...
		namespace NMNMRUNPARAMAPIFUNC
		{
			...
			*/
			NMUTIL_CLASS	NMRUNPARAM_RETCODE	SetDLLFileName(LPCSTR szFileName);
			...
			*/
			NMUTIL_CLASS	LPCSTR				ConvertRetCode2String(NMRUNPARAM_RETCODE retCode);
			...
			*/
			//NMUTIL_CLASS	LPCSTR				ConvertRunProgError2String(UINT uErr);
		};
		...
		namespace NMNMRUNPARAMAPIFUNC_ENVVAR
		{
			...
			*/
			NMUTIL_CLASS	LPCSTR				GetEnvName(NMRUNPARAMAPIFUNC_ENVVAR::ENVVARNAME eName);
			/**
			...
			*/
			NMUTIL_CLASS	NMRUNPARAM_RETCODE	GetEnvVar(LPCSTR szEnvName, LPSTR szBuffer, DWORD dwBufferLen);
			/**
			@brif - 주어진 환경 변수 이름 코드를 바탕으로 저장된 환경 변수를 가져온다.
			@returns 성공하였을 경우 @ref ERROR_NMRUNPARAM_SUCCESS 를 반환하고, 실패하였을 경우 @ref ERROR_NMRUNPARAM_SUCCESS 를 제외한 열거자 @ref ERROR_NMRUNPARAM 값들 중에 하나를 반환한다.
			@param[in] eName - 가져올 환경 변수 값의 환경 변수 이름 코드를 입력한다.
			...
			@param[in] dwBufferLen - 가져올 환경 변수 값이 저장될 버퍼의 길이를 입력한다.
			*/
			NMUTIL_CLASS	NMRUNPARAM_RETCODE	GetEnvVar(NMRUNPARAMAPIFUNC_ENVVAR::ENVVARNAME eName, LPSTR szBuffer, DWORD dwBufferLen);
			...
			@param[in] eName - 가져올 환경 변수 값의 환경 변수 이름 코드를 입력한다.
			*/
			NMUTIL_CLASS	DWORD				GetEnvVarLength(NMRUNPARAMAPIFUNC_ENVVAR::ENVVARNAME eName);
			/**
			...
			*/
			NMUTIL_CLASS	DWORD				GetEnvVarLength(LPCSTR szEnvName);
			...
			*/
			NMUTIL_CLASS	NMRUNPARAM_RETCODE	SetEnvVar(NMRUNPARAMAPIFUNC_ENVVAR::ENVVARNAME eName, LPCSTR szValue);
			/**
			...
			@param[in] szValue - 저장할 환경 변수 값을 입력한다.
			*/
			NMUTIL_CLASS	NMRUNPARAM_RETCODE	SetEnvVar(LPCSTR szEnvName, LPCSTR szValue);

			/**
			@brif - 사이트 코드 환경 변수 값을 가져온다. (미구현)
			@returns 사이트 코드 환경 변수 값을 반환한다.
			*/
			NMUTIL_CLASS	LPCSTR				GetSiteName();
			/**
			@brif - 사이트 코드 환경 변수 값을 설정한다. (미구현)
			@returns 성공하였을 경우 @ref ERROR_NMRUNPARAM_SUCCESS 를 반환하고, 실패하였을 경우 @ref ERROR_NMRUNPARAM_SUCCESS 를 제외한 열거자 @ref ERROR_NMRUNPARAM 값들 중에 하나를 반환한다.
			@param[in] szSiteName - 사이트 코드 환경 변수 값에 저장될 사이트 이름을 입력한다.
			*/
			NMUTIL_CLASS	NMRUNPARAM_RETCODE	SetSiteName(LPCSTR szSiteName);

			/**
			@brif - 공통 모듈 경로를 가져온다.
			@returns 공통 모듈 경로를 반환한다.
			*/
			NMUTIL_CLASS	LPCSTR				GetCommonPath();
			/**
			@brif - 공통 모듈 경로를 설정한다.
			@returns 성공하였을 경우 @ref ERROR_NMRUNPARAM_SUCCESS 를 반환하고, 실패하였을 경우 @ref ERROR_NMRUNPARAM_SUCCESS 를 제외한 열거자 @ref ERROR_NMRUNPARAM 값들 중에 하나를 반환한다.
			@param[in] szCommonPath - 설정할 공통 모듈 경로를 입력한다.
			*/
			NMUTIL_CLASS	NMRUNPARAM_RETCODE	SetCommonPath(LPCSTR szCommonPath);
		};
		...
		namespace NMNMRUNPARAMAPIFUNC_CMDARG
		{
			...
			*/
			NMUTIL_CLASS	NMRUNPARAM_RETCODE	GetArg(unsigned short iArgIdx, LPSTR szBuffer, DWORD dwBufferLen);
			/**
			@brif - 지시된 인덱스의 Command line Argument 값의 문자열 길이를 가져온다.
			@returns 지시된 인덱스의 Command line Argument 값의 문자열 길이를 반환한다.
			@param[in] iArgIdx - 가져올 Command line Argument 의 인덱스를 입력한다.
			*/
			NMUTIL_CLASS	DWORD				GetArgLength(unsigned short iArgIdx);
			...
			*/
			NMUTIL_CLASS	LPCSTR				GetRunFilePath();
			/**
			...
			*/
			NMUTIL_CLASS	LPCSTR				GetRunParamKey();

			/**
			@brif - 현재 프로세스의 Current Directory 를 실행파일 경로로 변환한다.
			@returns 성공하였을 경우 @ref ERROR_NMRUNPARAM_SUCCESS 를 반환하고, 실패하였을 경우 @ref ERROR_NMRUNPARAM_SUCCESS 를 제외한 열거자 @ref ERROR_NMRUNPARAM 값들 중에 하나를 반환한다.
			*/
			NMUTIL_CLASS	NMRUNPARAM_RETCODE	SetCurPathToRunFilePath();
		};
		...
		/// @brief 실행 파라미터 클래스
		class NMUTIL_CLASS CNMNMRunParam
		{
		protected:
			CNMRunParam	*m_pParam;
		public:
			/** @brif - CNMNMRunParam 클래스 생성자 */
			CNMNMRunParam();
			/** @brif - CNMNMRunParam 클래스 소멸자 */
			virtual ~CNMNMRunParam();
