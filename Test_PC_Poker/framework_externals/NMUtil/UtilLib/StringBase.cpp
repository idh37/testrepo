...
namespace NMBASE
{
	namespace UTIL
	{
		...
		BOOL CNMStr::Compare(CString strData)
		{
			if(strData==_T(""))
				return FALSE;
			if(m_strData==_T("") || m_nDataSize<=0) 
				return FALSE;

			if(m_strData.CompareNoCase(strData.operator LPCTSTR())==0) 
				return TRUE;
		...
		BOOL CNMStr::CompareID(CString strID)
		{
			if(strID==_T(""))
				return FALSE;
			if(m_strID==_T("") || m_nIDSize<=0) 
				return FALSE;

			if(m_strID.CompareNoCase(strID.operator LPCTSTR())==0) 
				return TRUE;
