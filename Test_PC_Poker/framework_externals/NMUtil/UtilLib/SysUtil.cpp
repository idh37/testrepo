...
namespace NMBASE
{
	namespace UTIL
	{
		...
		INT64 GetProfile_int64(LPCTSTR strSection, LPCTSTR strKeyname, INT64 nDefault)
		{
			// 실행 파일명을 알아낸다
			char path[MAX_PATH], fname[MAX_PATH];
			if(!GetProfile_DefPath(path, MAX_PATH-1))
		...
		BOOL WriteProfile_int64(LPCTSTR strSection, LPCTSTR strKeyname, INT64 nValue)
		{
			// 실행 파일명을 알아낸다
			char path[MAX_PATH], fname[MAX_PATH];
			if(!GetProfile_DefPath(path, MAX_PATH-1))
...
#define CHECK_PASS ("12o3ijhlkjdfalojh139123ujlnaskdhaleu129-3j3-u52i-1jn.,andlkasuyd01e13")
		...
		BOOL OpenNehompy(LPCTSTR szMyUniqNo, LPCTSTR szTargetID) 
		{
			...
			strcpy_s(notify.UserID, 13, szMyUniqNo);
			str.Format("CmdXFunc(\"1\", \"%s\")", szTargetID);
			strcpy_s(notify.TargetURL, 100, str);
			strcpy_s(notify.CheckPass, 100, CHECK_PASS);
		...
		BOOL OpenNetmarbleWebpage(LPCTSTR szMyUniqNo, LPCTSTR szTargetURL) 
		{
			...
			strcpy_s(notify.UserID, 13, szMyUniqNo);		// 내 유니크 넘버
			strcpy_s(notify.TargetURL, 100, szTargetURL);	// 이동할 URL
			strcpy_s(notify.CheckPass, 100, CHECK_PASS);
		...
		//// GraphicEngine에 Dependency가 없도록 한다.
