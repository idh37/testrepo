...
BOOL xSprite::Load(char *name, int ScreenRGBMode)
{
	...
	if(strncmp("93XYZ sprite", tempsfhead.Identifier, 12) != 0) 
	{
		//스프라이트 파일이 아니면
	...
	// 파일 버전 검사
	if(tempsfhead.Version < 30 || tempsfhead.Version > 31) 
	{
	...
	for(int i=0; i < sfhead.TotalSprNum; i++)
	{
		...
		if(spr[i].PixelFormat == 8888) 
		{
			// 알파 테이블 시작 포인터를 지정
			int alphasize = spr[i].xl * spr[i].yl * 4;
			spr[i].pAlpha = ((BYTE*)spr[i].data) + spr[i].dsize - alphasize;
		}
	}
...
void xSprite::OptimizeSize(int snum, BOOL fixPos)
{
	...
	if(spr[snum].PixelFormat == 8888)
	{
		alphasize = newXl * newYl *4;
	}
...
BOOL xSprite::Compress(LONG snum)
{
	...
	if(spr==NULL)
		return FALSE;
	if(snum < 0 || snum >= sfhead.TotalSprNum)
		return FALSE;
	if(spr[snum].data == NULL)
		return FALSE;
	if(spr[snum].compress == 1) 
		return FALSE;
	...
	else if(spr[snum].PixelFormat == 8888)
	{
		// 알파테이블을 포함한 스프라이트라면 새롭게 할당하여 이미지 복사
		int alphasize = spr[snum].xl * spr[snum].yl * 4;
...
BOOL xSprite::DeCompress(LONG snum)
{
	...
	{
		// 알파테이블을 포함한 스프라이트
		alphasize = spr[snum].xl * spr[snum].yl * 4;
	}
...
{
	int i,x,y;
	if(NewForm == sfhead.PixelFormat)
		return TRUE;

	if(NewForm!=555 && NewForm!=565 && NewForm!=-565) 
		return FALSE;
...
BOOL xSprite::Insert(LONG snum, int xl, int yl, WORD *pImage, WORD wColorKey, BOOL bCompress)
{
	...
	int i;
	for(i=0; i<snum; i++) 
		memcpy(spr+i, oldspr+i, sizeof(SPRITE30));
	
	for(i=snum+1; i<sfhead.TotalSprNum; i++) 
		memcpy(spr+i, oldspr+i-1, sizeof(SPRITE30));
