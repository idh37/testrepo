...
BOOL CPage::SaveToBitmap(LPSTR fname)
{
	...
	for(int i=height-1; i>=0; i--) 
	{
		...
		{
			//WORD color = *((WORD*)lpBit + (i * lPitch + j));
			// 주의) BGR순이다
			*(bit+(j*3))   = sorbit[(i * lPitch + (j * 3)) +0] ;	// B
			*(bit+(j*3)+1) = sorbit[(i * lPitch + (j * 3)) +1];
			*(bit+(j*3)+2) = sorbit[(i * lPitch + (j * 3)) +2];
		}
		bit += npitch;
	}
...
BOOL CPage::SaveToJpeg(LPSTR fname, int quality)
{
	...
	for(int i=0; i<height; i++) 
	{
		for(int j=0; j<width; j++) 
		{
			...
			// RGB
			*(bit+(j*3))   = sorbit[(i * lPitch + (j * 3)) +2] ;	// B
			*(bit+(j*3)+1) = sorbit[(i * lPitch + (j * 3)) +1];
			*(bit+(j*3)+2) = sorbit[(i * lPitch + (j * 3)) +0];
		}
		bit += npitch;
	}
...
BOOL CPage::ZoomSprAuto(int x, int y, SPRITE30 *pSpr, int tx, int ty, int opt, int grade, WORD color)
{
	...
	{
		if(opt==NULL) _ZoomSpr(x, y, pSpr->xl, pSpr->yl, (WORD*)lpBit, tx, ty, pSpr->data, pSpr->colorkey);
