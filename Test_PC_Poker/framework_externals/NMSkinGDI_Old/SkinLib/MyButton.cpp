...
#include "stdafx.h"
#include "MyButton.h"
//#include "../Libraries/SoundLib/Sound.h"
//#include "Global.h"
	...
	namespace SKINGDI
	{
		...
		void CMyButton::Draw(CDC& dc) 
		{
			...
			*/
				...
				if(bOneClip) {
					...
					if(!bCheckbtn) {
						...
						if( iUser1 == 0 ){
							...
							int c = 3;				
							if(!Clip[c].bInit) c = 0;
							if(Clip[c].bInit) dc.BitBlt(0, 0, Clip[c].cx, Clip[c].cy, &cdc, Clip[c].x, Clip[c].y, dwRop);
						}
						else{
							int c = 3;
							if(Clip[c].bInit){
								BmpOne.TransDraw(dc.m_hDC,0, 0, BmpOne.GetWidth(), BmpOne.GetHeight()/4, Clip[c].x, Clip[c].y,RGB(255,0,255));
							}				
						}				
					}
					else {
						// 체크버튼 스타일인 경우
						int c = 0;	// 0:Unchecked 1:Checked 2:Unchecked Disable 3:Checked Disable
					...
					if(!bCheckbtn) {
						...
						dc.BitBlt(0, 0, rect.right, rect.bottom, &cdc, 0, 0, dwRop);
					}
					else {
						// 체크버튼 스타일인 경우
						if(Bmp1.m_hObject==NULL && Bmp4.m_hObject==NULL) return;
						...
						if(GetCheck() == 1) {
							// 체크된 상태로 Disable
							if(Bmp2.m_hObject==NULL && Bmp4.m_hObject==NULL) return;
							cdc.SelectObject((Bmp4.m_hObject!=NULL) ? &Bmp4 : &Bmp2);
							dc.BitBlt(0, 0, rect.right, rect.bottom, &cdc, 0, 0, dwRop);
						}
						else {
							// 체크 안된 상태로 Disable
							if(Bmp1.m_hObject==NULL && Bmp3.m_hObject==NULL) return;
							cdc.SelectObject((Bmp3.m_hObject!=NULL) ? &Bmp3 : &Bmp1);
							dc.BitBlt(0, 0, rect.right, rect.bottom, &cdc, 0, 0, dwRop);
						}
					}
				}			
				...
				case 1:	// 보통 상태
					...
					if(bOneClip) {				
						...
						if( iUser1 == 0 ){
							...
							int c = 0;
							if(Clip[c].bInit){
								dc.BitBlt(0, 0, Clip[c].cx, Clip[c].cy, &cdc, Clip[c].x, Clip[c].y, dwRop);						
							}
						}
						else{
							int c = 0;
			...
			if( cdc.GetSafeHdc() )cdc.DeleteDC();
		...
		void CMyButton::OnLButtonDown(UINT nFlags, CPoint point) 
		{
			...
			{
			Sound.PlayWav(m_strClickSoundID);
			}*/
		...
		void CMyButton::OnMouseMove(UINT nFlags, CPoint point) 
		{
			...
			else
			{
				if(!m_bMouseOver)
				{
					...
					/*if(m_strOverSoundID != "") 
					{
					Sound.PlayWav(m_strOverSoundID);
					}*/
				}

				if(nState==1) 
				{ 
					nState=2; Invalidate(FALSE);
				}

				if(nState!=3 && bClicked)
				{
					nState=3; Invalidate(FALSE);
				}
			}
		...
		void CMyButton::Init(UINT bmp1, UINT bmp2, UINT bmp3, UINT bmp4, char* wavfile)
		{
			...
			if((btnstyle & BS_OWNERDRAW) != BS_OWNERDRAW) {
				// 오너 드로우 속성이 아닌 경우만 체크박스(라디오 버튼)가 될 수 있다
				if((btnstyle & BS_CHECKBOX) == BS_CHECKBOX || 
					(btnstyle & BS_AUTOCHECKBOX) == BS_AUTOCHECKBOX || 
					(btnstyle & BS_RADIOBUTTON) == BS_RADIOBUTTON ||
					(btnstyle & BS_AUTORADIOBUTTON) == BS_AUTORADIOBUTTON)
					bCheckbtn = TRUE;
			}
		...
		void CMyButton::InitRegion(int xp, int yp, LPCTSTR filename, int imagenum, LPCTSTR wavfile,BOOL bRegion)//InitColorkey(int xp, int yp, LPCTSTR filename, int imagenum, LPCTSTR wavfile,BOOL bColorkey)
		{
			iUser1 = bRegion;
			Init(xp,yp,filename,imagenum,wavfile);	
			if( BmpOne.m_hObject ){
				//	SetRgn( HBITMAP hBitmap, CDialog* pDlg, COLORREF transcolor, int bmp_height, int bmp_width  )
				m_Rgn.SetRgn( HBITMAP(BmpOne), (CDialog*)this, RGB( 255, 0, 255 ), BmpOne.GetHeight() / 4,BmpOne.GetWidth() );	
			}	
		}
		...
		{
			bEnable = enable;
			if(enable == FALSE)	m_bMouseOver = false;
		...
		void CMyButton::OnShowWindow(BOOL bShow, UINT nStatus) 
		{
			...
			if(bShow == FALSE) m_bMouseOver = false;
		...
		void CMyButton::OnDestroy() 
		{
			...
			BmpOne.DeleteObject();	// 통짜 모드일 경우
			Bmp1.DeleteObject();
			Bmp2.DeleteObject();
			Bmp3.DeleteObject();
			Bmp4.DeleteObject(); // 첫번째 버튼 그림, 마우스가 위를 지날때, 눌려졌을때, Disable되었을때
			// TODO: Add your message handler code here

		}

		BOOL CMyButton::SetPos(int x, int y)
		{
			return CButton::SetWindowPos(NULL, x, y, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
		}
