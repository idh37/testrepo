...
CRoomListHeaderLowBDJoin::CRoomListHeaderLowBDJoin(void)
:	m_pTabRoomType(NULL),
...
void CRoomListHeaderLowBDJoin::OnCreateChild(LSY::CObjectBase *pObject)
{
	...
	{
	case ID_LOBBY_TAB_ROOM_LIST_LOWBD_JOIN:				m_pTabRoomType					= (LSY::CTabControl *)pObject;		break;
	case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_NO_LOWBD_JOIN:		m_pBtnRoomNumber			= (LSY::CTextButton *)pObject;	bAddChild = true;	break;
	case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_MINMONEY_LOWBD_JOIN:	m_pBtnMinimumMoney		= (LSY::CTextButton *)pObject;	bAddChild = true;	break;
	case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_TITLE_LOWBD_JOIN:		m_pBtnTitle				= (LSY::CTextButton *)pObject;	bAddChild = true;	break;
	case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_SEEDMONEY_LOWBD_JOIN:	m_pBtnSeedMoney			= (LSY::CTextButton *)pObject;	bAddChild = true;	break;
	case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_USERCOUNT_LOWBD_JOIN:	m_pBtnUserCount			= (LSY::CTextButton *)pObject;	bAddChild = true;	break;
	case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_DEFAULT_SORT_LOWBD_JOIN: m_pBtnDefaultSort		= (LSY::CTextButton *)pObject;	bAddChild = true;	break;
...
//	case ID_LOBBY_BTN_ROOMLIST_HEADER_DEFAULT_SORT_LOWBD:	m_pBtnDefaultSort		= (LSY::CButton *)pObject;		bAddChild = true;	break;
	case ID_LOBBY_LIST_ROOM_LOWBD_JOIN:							m_pList					= (LSY::CList *)pObject;			break;
	}

	if(bAddChild)
	{
		pObject->AddHandler(LSY::EM_O_MOUSELCLICK, LSY::Fnt(this, &CRoomListHeaderLowBDJoin::OnClickButton));
	}

	__super::OnCreateChild(pObject);
}

void CRoomListHeaderLowBDJoin::OnCreated(void)
{
	...
	m_pTabRoomType->AddHandler(LSY::EM_O_SELCHANGE, LSY::Fnt(this, &CRoomListHeaderLowBDJoin::OnRoomTypeSelChange));

	m_pList->AddHandler(LSY::EM_O_LISTITEM_CLICK, LSY::Fnt(this, &CRoomListHeaderLowBDJoin::OnListItemLClick));
	m_pList->AddHandler(LSY::EM_O_LISTITEM_DBCLICK, LSY::Fnt(this, &CRoomListHeaderLowBDJoin::OnListItemLDbClick));

	LSY::CListItem *pListItem = m_pList->GetItem();
	LSY::CButton *pEnterRoom = (LSY::CButton *)pListItem->GetFirstChildFromID(ID_LOBBY_BTN_ROOMLISTITEM_ENTER_ROOM_LOWBD_JOIN);
	pEnterRoom->AddHandler(LSY::EM_O_MOUSELCLICK, LSY::Fnt(this, &CRoomListHeaderLowBDJoin::OnClickEnterRoom));

	LSY::CButton *pObserverRoom = (LSY::CButton *)pListItem->GetFirstChildFromID(ID_LOBBY_BTN_ROOMLISTITEM_OBSERVER_ROOM_LOWBD_JOIN);
	pObserverRoom->AddHandler(LSY::EM_O_MOUSELCLICK, LSY::Fnt(this, &CRoomListHeaderLowBDJoin::OnClickObserverRoom));

	ChangeRoomTab(ROOMLIST_TYPE_NORMAL);

	LSY::CSprite* pcSprite = (LSY::CSprite*)m_pTabRoomType->GetFirstChildFromID(ID_LOBBY_SPRITE_PROMOTION_VIP_COUPON_LOWBD_JOIN);
	if(NULL!=pcSprite){
...
LRESULT CRoomListHeaderLowBDJoin::OnClickButton(LSY::CMessage *msg)
{
	...
	switch(pBtn->GetID())
	{
		case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_NO_LOWBD_JOIN:
		{
		...
		case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_MINMONEY_LOWBD_JOIN:
		{
			#ifdef UPGRADE_10210619_ADD_CLICKLOG
		...
		case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_TITLE_LOWBD_JOIN:
		{
			#ifdef UPGRADE_10210619_ADD_CLICKLOG
		...
		case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_SEEDMONEY_LOWBD_JOIN:
		{
			#ifdef UPGRADE_10210619_ADD_CLICKLOG
		...
		case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_USERCOUNT_LOWBD_JOIN:
		{
			#ifdef UPGRADE_10210619_ADD_CLICKLOG
		...
		case ID_LOBBY_TXTBTN_ROOMLIST_HEADER_DEFAULT_SORT_LOWBD_JOIN:
		{
			#ifdef UPGRADE_10210619_ADD_CLICKLOG
...
void CRoomListHeaderLowBDJoin::Sort(int nSortType)
{
...
void CRoomListHeaderLowBDJoin::SortDefault(void)
{
...
void CRoomListHeaderLowBDJoin::SortEnterRoomMinimumMoney(INT64 *i64Money)
{
...
void CRoomListHeaderLowBDJoin::ChangeRoomTab( ROOMLIST_TYPE eRoomListType )
{
	UINT nID = ID_LOBBY_RAD_VIP_ROOM_LIST_LOWBD_JOIN;

	if (eRoomListType==ROOMLIST_TYPE_NORMAL)
	{
		GM().SetRoomListTabState(1) ;
		nID = ID_LOBBY_RAD_NORMAL_ROOM_LIST_LOWBD_JOIN;
	}
	else if (eRoomListType==ROOMLIST_TYPE_FREE)
	{
		GM().SetRoomListTabState(0) ;
		nID = ID_LOBBY_RAD_FREE_ROOM_LIST_LOWBD_JOIN;
	}
	else if (eRoomListType==ROOMLIST_TYPE_VIP_GOLD)
	{
		GM().SetRoomListTabState(2) ;
		nID = ID_LOBBY_RAD_VIP_GOLD_ROOM_LIST_LOWBD_JOIN;
	}
	else if (eRoomListType==ROOMLIST_TYPE_NORMAL_GOLD)
	{
		GM().SetRoomListTabState(2) ;
		nID = ID_LOBBY_RAD_NORMAL_GOLD_ROOM_LIST_LOWBD_JOIN;
	}
	else
	{
		GM().SetRoomListTabState(1) ;
...
void CRoomListHeaderLowBDJoin::AddDataRoomList()
{
	RemoveAll();
	CDataRoomList::DATAPOSITION pos;
	SMALLROOMINFO *pRoomInfo = GM().GetDataRoomList()->GetFirstData(pos);
...
LRESULT CRoomListHeaderLowBDJoin::OnRoomTypeSelChange(LSY::CMessage *msg)
{
...
void CRoomListHeaderLowBDJoin::SetJoinable( bool bstate )
{
	m_bJoinable = bstate;
}


void CRoomListHeaderLowBDJoin::RemoveAll(void)
{
	m_pList->DeleteAllItem();
}

void CRoomListHeaderLowBDJoin::AddItem(SMALLROOMINFO *pRoomInfo)
{
...
void CRoomListHeaderLowBDJoin::DelItem(SMALLROOMINFO *pRoomInfo)
{
	m_pList->DeleteItem((LPARAM)pRoomInfo);
}

void CRoomListHeaderLowBDJoin::ChangeItem(SMALLROOMINFO *pRoomInfo)
{
	m_pList->UpdateItemFromParam((LPARAM)pRoomInfo);
}

void CRoomListHeaderLowBDJoin::SetListEnable(bool bEnable)
{
	m_pList->SetEnable(bEnable);
}

LRESULT CRoomListHeaderLowBDJoin::OnListItemLClick(LSY::CMessage *msg)
{
...
LRESULT CRoomListHeaderLowBDJoin::OnListItemLDbClick(LSY::CMessage *msg)
{
...
LRESULT CRoomListHeaderLowBDJoin::OnClickEnterRoom(LSY::CMessage *msg)
{
...
LRESULT CRoomListHeaderLowBDJoin::OnClickObserverRoom(LSY::CMessage *msg)
{
...
void CRoomListHeaderLowBDJoin::RemoveSelectedRoom(void)
{
...
void CRoomListHeaderLowBDJoin::ChangeRoomList()
{
